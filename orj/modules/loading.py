...
import orj
import orj.modules.db
import orj.modules.graph
import orj.modules.migration
import orj.modules.registry
from .. import SUPERUSER_ID, api, tools
from .module import adapt_version, initialize_sys_path, load_orj_module
...
def load_module_graph(env, graph, status=None, perform_checks=True,
    ...
    migrations = orj.modules.migration.MigrationManager(env.cr, graph)
    module_count = len(graph)
    _logger.info('loading %d modules...', module_count)

    # register, instantiate and initialize models for each modules
    t0 = time.time()
    loading_extra_query_count = orj.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = orj.sql_db.sql_counter
        ...
        load_orj_module(package.name)

        if new_install:
            py_module = sys.modules['orj.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = orj.tools.config["overwrite_existing_translations"]
        ...
        if tools.config.options['test_enable'] and (needs_update or not updating):
            ...
            if suite.countTestCases():
                if not needs_update:
                    registry.setup_models(env.cr)
                # Python tests
                tests_t0, tests_q0 = time.time(), orj.sql_db.sql_counter
                test_results = loader.run_suite(suite, global_report=report)
                report.update(test_results)
                test_time = time.time() - tests_t0
                test_queries = orj.sql_db.sql_counter - tests_q0
        ...
        extra_queries = orj.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   orj.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
...
def load_modules(registry, force_demo=False, status=None, update_module=False):
    ...
    with registry.cursor() as cr:
        ...
        if not orj.modules.db.is_initialized(cr):
        ...
        if update_module and tools.config['update']:
            for pyfile in tools.config['pre_upgrade_scripts'].split(','):
                orj.modules.migration.exec_script(cr, graph['base'].installed_version, pyfile, 'base', 'pre')

        if update_module and orj.tools.sql.table_exists(cr, 'ir_model_fields'):
        ...
        migrations = orj.modules.migration.MigrationManager(cr, graph)
        for package in graph:
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['orj.addons.%s' % (pkg.name,)]
                ...
                registry = orj.modules.registry.Registry.new(
