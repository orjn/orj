...
import orj
from orj.modules.db import FunctionStatus
from .. import SUPERUSER_ID
from orj.sql_db import TestCursor
from orj.tools import (
    config, lazy_classproperty,
    lazy_property, sql, OrderedSet, SQL,
    remove_accents,
)
from orj.tools.func import locked
from orj.tools.lru import LRU
from orj.tools.misc import Collector, format_frame

if typing.TYPE_CHECKING:
    from orj.models import BaseModel


_logger = logging.getLogger(__name__)
_schema = logging.getLogger('orj.schema')
...
class Registry(Mapping):
    ...
    def new(cls, db_name, force_demo=False, status=None, update_module=False):
        ...
        try:
            ...
            try:
                orj.modules.load_modules(registry, force_demo, status, update_module)
            except Exception:
                orj.modules.reset_modules_state(db_name)
                raise
        except Exception:
    ...
    def init(self, db_name):
        ...
        if config['test_enable'] or config['test_file']:
            from orj.tests.result import OrjTestResult  # noqa: PLC0415
            self._assertion_report = OrjTestResult()
        else:
        ...
        self._db = orj.sql_db.db_connect(db_name, readonly=False)
        ...
        if config['db_replica_host'] is not False or config['test_enable']:  # by default, only use readonly pool if we have a db_replica_host defined. Allows to have an empty replica host for testing
            self._db_readonly = orj.sql_db.db_connect(db_name, readonly=True)
        ...
        with closing(self.cursor()) as cr:
            self.has_unaccent = orj.modules.db.has_unaccent(cr)
            self.has_trigram = orj.modules.db.has_trigram(cr)
    ...
    def init_models(self, cr, model_names, context, install=True):
        ...
        env = orj.api.Environment(cr, SUPERUSER_ID, context)
    ...
    def cursor(self, /, readonly=False):
        ...
        if self.test_cr is not None:
            ...
            return TestCursor(self.test_cr, self.test_lock, readonly and self.test_readonly_enabled, current_test=orj.modules.module.current_test)
