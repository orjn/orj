...
class Shell(Command):
    """Start orj in an interactive shell"""
    ...
    def init(self, args):
        ...
        orj.cli.server.report_configuration()
        orj.service.server.start(preload=[], stop=True)
    ...
    def shell(self, dbname):
        ...
        if dbname:
            ...
            with registry.cursor() as cr:
                ...
                ctx = orj.api.Environment(cr, uid, {})['res.users'].context_get()
                env = orj.api.Environment(cr, uid, ctx)
                local_vars['env'] = env
