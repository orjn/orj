...
import orj
from orj.tools import config, topological_sort, unique
from orj.modules.registry import Registry
from orj.netsvc import init_logger
from orj.tests import standalone_tests
import orj.tests.loader

_logger = logging.getLogger('orj.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Orj modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra Orj modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        orj.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        orj.tools.config['addons_path'] = ','.join([args.addons_path, orj.tools.config['addons_path']])
        if args.data_dir:
            orj.tools.config['data_dir'] = args.data_dir
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'orj.modules.loading': {'level': 'CRITICAL'},
            'orj.sql_db': {'level': 'CRITICAL'},
            'orj.models.unlink': {'level': 'WARNING'},
            'orj.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
