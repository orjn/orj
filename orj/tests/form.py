# -*- coding: utf-8 -*-
"""
The module :mod:`orj.tests.form` provides an implementation of a client form
...
class Form:
    ...
    * properly handle defaults & onchanges around x2many fields.
    ...
    of :class:`~orj.fields.Many2one` fields, one can assign a recordset::
    ...
    For :class:`~orj.fields.Many2many` fields, the field itself is a
    :class:`~orj.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~orj.fields.One2many` are reified as :class:`~O2MProxy`.

    Because the :class:`~orj.fields.One2many` only exists through its parent,
    ...
    @classmethod
    def from_action(cls, env: orj.api.Environment, action: dict) -> Form:
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~orj.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~orj.fields.One2many` record.
...
def convert_read_to_form(values, fields):
    ...
    for fname, value in values.items():
        ...
        if field_info['type'] == 'one2many':
            ...
            value = orj.fields.Datetime.to_string(value)
        elif field_info['type'] == 'date' and isinstance(value, date):
            value = orj.fields.Date.to_string(value)
        result[fname] = value
    return result
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return orj.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return orj.fields.Date.to_string(value)
    return value
