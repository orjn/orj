...
from .suite import OrjSuite
from .result import OrjTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'orj.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"orj.upgrade.{module}",
        f"orj.addons.{module}.migrations",
        f"orj.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return OrjSuite(sorted(tests, key=lambda t: t.test_sequence))
...
def run_suite(suite, global_report=None):
    ...
    results = OrjTestResult(global_report=global_report)
    suite(results)
