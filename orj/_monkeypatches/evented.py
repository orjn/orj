...
It will initialize the `orj.evented` variable.
"""
import orj
import sys
...
def patch_evented():
    if orj.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in orj/debian/copyright)
        while 1:
            state = conn.poll()
