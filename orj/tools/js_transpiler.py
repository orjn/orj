"""
This code is what let us use ES6-style modules in orj.
Classic Orj modules are composed of a top-level :samp:`orj.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@orj-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** orj-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual orj one with minimal changes.
...
from orj.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom orj modules.
    ...
    legacy_orj_define = get_aliased_orj_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_orj_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_orj_define:
        content += legacy_orj_define
    return content
...
def url_to_module_path(url):
    """
    Orj modules each have a name. (orj.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_orj_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the orj.define call.
    ...
    """
    return f"""orj.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
ORJ_MODULE_RE = re.compile(r"""
    \s*                             # starting white space
    \/(\*|\/)                       # /* or //
    .*                              # any comment in between (optional)
    @orj-module                    # '@orj-module' statement
...
def is_orj_module(url, content):
    """
    Detect if the file is a native orj module.
    We look for a comment containing @orj-module.

    :param url:
    :param content: source code
    :return: is this a orj module that need transpilation ?
    """
    result = ORJ_MODULE_RE.match(content)
...
def get_aliased_orj_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the orj source base being:
    ...
    > orj.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @orj-module */
        // after
        /** @orj-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @orj-module */
        // after
        /** @orj-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = ORJ_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\norj.define(`%s`, ['%s'], function (require) {
                        return require('%s');
