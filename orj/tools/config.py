...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.orj.net/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.orjrc (or to ~/.orj_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Orj data")
        ...
        group.add_option("--gevent-port", dest="gevent_port", my_default=8073,
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'orj_tests')
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "orj.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="orj.http:DEBUG", help='shortcut for --log-handler=orj.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="orj.sql_db:DEBUG", help='shortcut for --log-handler=orj.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Orj to another language. "
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes orj.tools.config and orj.conf (the
        ...
        Typical usage of this method:
            ...
            orj.tools.config.parse_config(sys.argv[1:])
        """
        opt = self._parse_config(args)
        if setup_logging is not False:
            orj.netsvc.init_logger()
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of Orj 18, it's recommended to specify whether"
                    " you want Orj to setup its own logging (or want to"
                    " handle it yourself)",
                    category=PendingDeprecationWarning,
                    stacklevel=2,
                )
        self._warn_deprecated_options()
        orj.modules.module.initialize_sys_path()
        return opt
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'orj.conf')
        else:
            rcfilepath = os.path.expanduser('~/.orjrc')
            old_rcfilepath = os.path.expanduser('~/.orj_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.orjrc' and '.orj_serverrc' in your path. Please keep only one of "\
                "them, preferably '.orjrc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('ORJ_RC') or os.environ.get('ORJ_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from orj.modules.module import MANIFEST_NAMES
