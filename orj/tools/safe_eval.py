...
Orj domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None):
    ...
    an Orj domain expression from an untrusted source.
    ...
    except orj.exceptions.UserError:
        raise
    except orj.exceptions.RedirectWarning:
        raise
...
`orj.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `orj.tools.safe_eval`.
""")
    return d
...
from orj._monkeypatches.pytz import patch_pytz  # noqa: E402, F401
patch_pytz()
