...
"""
Miscellaneous tools used by Orj.
"""
...
from orj.loglevels import exception_to_unicode, get_encodings, ustr  # noqa: F401
...
if typing.TYPE_CHECKING:
    ...
    from orj.addons.base.models.res_lang import LangData
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if orj.tools.config['db_host']:
        env['PGHOST'] = orj.tools.config['db_host']
    if orj.tools.config['db_port']:
        env['PGPORT'] = str(orj.tools.config['db_port'])
    if orj.tools.config['db_user']:
        env['PGUSER'] = orj.tools.config['db_user']
    if orj.tools.config['db_password']:
        env['PGPASSWORD'] = orj.tools.config['db_password']
    return env
...
def file_path(file_path: str, filter_ext: tuple[str, ...] = ('',), env: Environment | None = None) -> str:
    ...
    addons_paths = [*orj.addons.__path__, root_path, *temporary_paths]
...
def file_open(name: str, mode: str = "r", filter_ext: tuple[str, ...] = (), env: Environment | None = None):
    ...
    Examples::
        ...
        >>> with file_open('/opt/orj/addons/hr/static/description/icon.png', 'rb') as f:
        ...     contents = f.read()
...
def file_open_temporary_directory(env: Environment):
    ...
    Examples::
        ...
        >>> with orj.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with orj.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def scan_languages() -> list[tuple[str, str]]:
    """ Returns all languages supported by Orj for translation
...
class mute_logger(logging.Handler):
    ...
    Can be used as context manager or decorator::
        ...
        @mute_logger('orj.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('orj.foo.bar'):
            do_suff()
    """
...
def format_date(
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = orj.fields.Datetime.from_string(value)
            value = orj.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = orj.fields.Datetime.from_string(value)
...
def format_datetime(
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = orj.fields.Datetime.from_string(value)
    else:
        timestamp = value
...
def limited_field_access_token(record, field_name, timestamp=None):
    ...
    :type record: class:`orj.models.Model`
