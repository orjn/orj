...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "orj.netsvc.log is deprecated starting Orj 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('orj.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('orj.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='orj.tools.image')
    ...
    # Check that handler.stream has a fileno() method: when running Orj
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("ORJ_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'orj.http.rpc.request:INFO',
    'orj.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['orj:DEBUG', 'orj.sql_db:INFO', 'orj.http.rpc:DEBUG'],
    'debug_rpc': ['orj:DEBUG', 'orj.sql_db:INFO', 'orj.http.rpc.request:DEBUG'],
    'debug': ['orj:DEBUG', 'orj.sql_db:INFO'],
    'debug_sql': ['orj.sql_db:DEBUG'],
    'info': [],
    'runbot': ['orj:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['orj:WARNING', 'werkzeug:WARNING'],
    'error': ['orj:ERROR', 'werkzeug:ERROR'],
    'critical': ['orj:CRITICAL', 'werkzeug:CRITICAL'],
}
