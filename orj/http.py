...
r"""\
Orj HTTP layer / WSGI application
...
:class:`~orj.http.Controller`: class and its companion the
:func:`~orj.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Orj http request. The Orj http request is
...
Orj URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.orj.net/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Orj via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~orj.service.db.list_dbs`:
    ...
    """
    try:
        dbs = orj.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Orj will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': orj.service.common.dispatch,
        'db': orj.service.db.dispatch,
        'object': orj.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~orj.tools.file_path`
        :param filter_ext: See :func:`~orj.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`orj.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~orj.http.Controller` can use the :func:`~orj.http.route`:
    ...
    :func:`~orj.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(orj.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['orj', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~orj.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['orj', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Orj addons.
        """
        # Controllers defined outside of orj addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of orj addons can be extended in
...
class Session(collections.abc.MutableMapping):
    ...
    def authenticate(self, dbname, credential):
        ...
        with registry.cursor() as cr:
            env = orj.api.Environment(cr, pre_uid, {})
        ...
        if request and request.session is self and request.db == dbname:
            request.env = orj.api.Environment(request.env.cr, self.uid, self.context)
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~orj.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> orj_ip = socket.gethostbyname('orj.net')
        >>> GeoIP(orj_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~orj.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~orj.addons.base.models.res_users.Users>`
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = orj.api.Environment(cr_readwrite, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to orj.http.Application.__call__
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~orj.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Orj Session Expired"
...
class Application:
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in orj.addons.__path__:
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint in _generate_routing_rules([''] + orj.conf.server_wide_modules, nodb_only=True):
    ...
    def session_store(self):
        path = orj.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if orj.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/orj/')
                            or httprequest.path in (
                                '/orj', '/web', '/web/login', '/test_http/ensure_db',
                            )):
