...
from orj.release import version_info
from orj.sql_db import db_connect
from orj.tools import SQL
from orj.tools.misc import exec_pg_environ, find_pg_tool
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not orj.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and orj.tools.config.verify_admin_password(passwd):
        return True
    raise orj.exceptions.AccessDenied()

# This should be moved to orj.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = orj.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            orj.modules.db.initialize(cr)
            orj.tools.config['load_language'] = lang
            cr.commit()

        registry = orj.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(registry.cursor()) as cr:
            env = orj.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = orj.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = orj.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = orj.tools.config['db_template']
    ...
    try:
        db = orj.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if orj.tools.config['unaccent']:
                ...
                # But in the case of Orj, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    orj.sql_db.close_db(db_original_name)
    db = orj.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = orj.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = orj.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            orj.modules.neutralize.neutralize_database(cr)

    from_fs = orj.tools.config.filestore(db_original_name)
    to_fs = orj.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    orj.modules.registry.Registry.delete(db_name)
    orj.sql_db.close_db(db_name)
    ...
    fs = orj.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'orj_dump': '1',
        'db_name': cr.dbname,
        'version': orj.release.version,
        'version_info': orj.release.version_info,
        'major_version': orj.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = orj.tools.config.filestore(db_name)
            ...
            if stream:
                orj.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                orj.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = orj.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = orj.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    orj.modules.registry.Registry.delete(old_name)
    orj.sql_db.close_db(old_name)
    ...
    old_fs = orj.tools.config.filestore(old_name)
    new_fs = orj.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    orj.tools.config.set_admin_password(new_password)
    orj.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        orj.tools.config['update']['base'] = True
        orj.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@orj.tools.mute_logger('orj.sql_db')
...
def list_dbs(force=False):
    if not orj.tools.config['list_db'] and not force:
        raise orj.exceptions.AccessDenied()

    if not orj.tools.config['dbfilter'] and orj.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Orj will not
        ...
        res = sorted(db.strip() for db in orj.tools.config['db_name'].split(','))
        return res
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Orj
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if orj.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        orj.sql_db.close_db(database_name)
...
def exp_list_lang():
    return orj.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(orj.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
