...
import orj
from orj.modules import get_modules
from orj.modules.registry import Registry
from orj.release import nt_service_name
from orj.tools import config
from orj.tools.cache import log_ormcache_stats
from orj.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    if orj.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'orj.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The ORJ_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("ORJ_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in orj.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="orj.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from orj.addons.base.models.ir_cron import ir_cron
        def _run_cron(cr):
            pg_conn = cr._cnx
            ...
            while config['limit_time_worker_cron'] <= 0 or (time.monotonic() - alive_time) <= config['limit_time_worker_cron']:
                ...
                registries = orj.modules.registry.Registry.registries
        ...
        while True:
            conn = orj.sql_db.db_connect('postgres')
    ...
    def cron_spawn(self):
        ...
        for i in range(orj.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="orj.service.cron.cron%d" % i)
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from orj.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('orj: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(orj.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The ORJ_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("ORJ_HTTP_SOCKET_TIMEOUT")
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = orj.service.db.list_dbs(True)
        return db_names
    ...
    def start(self):
        ...
        dbconn = orj.sql_db.db_connect('postgres')
...
def load_server_wide_modules():
    server_wide_modules = list(orj.conf.server_wide_modules)
    server_wide_modules.extend(m for m in ('base', 'web') if m not in server_wide_modules)
    for m in server_wide_modules:
        try:
            orj.modules.module.load_orj_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
The `web` module is provided by the addons found in the `orj-web` project.
Maybe you forgot to add those addons in your addons_path configuration."""
            _logger.exception('Failed to load server-wide module `%s`.%s', m, msg)

def _reexec(updated_modules=None):
    """reexecute orj-server process with (nearly) the same arguments"""
    if orj.tools.osutil.is_running_as_nt_service():
...
def load_test_file_py(registry, test_file):
    ...
    from orj.tests import loader  # noqa: PLC0415
    from orj.tests.suite import OrjSuite  # noqa: PLC0415
            ...
            if config['test_enable']:
                ...
                if post_install_suite.has_http_case():
                    with registry.cursor() as cr:
                        env = orj.api.Environment(cr, orj.SUPERUSER_ID, {})
...
def start(preload=None, stop=False):
    """ Start the orj http server and cron processor.
    """
    global server
    ...
    if orj.evented:
        server = GeventServer(orj.http.root)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(orj.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Orj is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(orj.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not orj.evented:
        if inotify:
