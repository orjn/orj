...
import orj
import orj.modules.module  # get_manifest, don't from-import it
from orj import api, fields, models, tools
from orj.tools import misc
from orj.tools.constants import ASSET_EXTENSIONS, EXTERNAL_ASSET
...
class IrAsset(models.Model):
    ...
    def _fill_asset_paths(self, bundle, asset_paths, seen, addons, installed, **assets_params):
        ...
        for addon in addons:
            for command in orj.modules.module._get_manifest_cached(addon)['assets'].get(bundle, ()):
    ...
    def _topological_sort(self, addons_tuple):
        ...
        def mapper(addon):
            manif = orj.modules.module._get_manifest_cached(addon)
    ...
    def _get_installed_addons_list(self):
        ...
        return self.env.registry._init_modules.union(orj.conf.server_wide_modules or [])
    ...
    def _get_paths(self, path_def, installed):
        ...
        Ex: ('/base/static/file.js', '/home/user/source/orj/orj/addons/base/static/file.js', 643636800)
        ...
        addon = path_parts[0]
        addon_manifest = orj.modules.module._get_manifest_cached(addon)
