...
from orj import api, fields, models, tools, _, modules
from orj.exceptions import UserError
from orj.tools import formataddr, email_normalize, encapsulate_email, email_domain_extract, email_domain_normalize, human_size
...
class IrMailServer(models.Model):
    ...
    from_filter = fields.Char(
        ...
        help='Comma-separated list of addresses or domains for which this server can be used.\n'
             'e.g.: "notification@orj.net" or "orj.net"')
    ...
    def connect(self, host=None, port=None, user=None, password=None, encryption=None,
        ...
        """Returns a new SMTP connection to the given SMTP server.
           ...
           :param ssl_certificate: filename of the SSL certificate used for authentication
               Used when no mail server is given and overwrite  the orj-bin argument "smtp_ssl_certificate"
           :param ssl_private_key: filename of the SSL private key used for authentication
               Used when no mail server is given and overwrite  the orj-bin argument "smtp_ssl_private_key"
        ...
        if smtp_encryption == 'ssl':
            if 'SMTP_SSL' not in smtplib.__all__:
                raise UserError(
                    _("Your Orj Server does not support SMTP-over-SSL. "
        ...
        # Store the "from_filter" of the mail server / orj-bin argument to  know if we
    ...
    def _find_mail_server(self, email_from, mail_servers=None):
        ...
        - Mail server to use to send the email (None if we use the orj-bin arguments)
        ...
        # Skip if passed email_from is False (example Orjbot has no email address)
        ...
        # 5: SMTP config in orj-bin arguments
        from_filter = self.env['ir.mail_server']._get_default_from_filter()
