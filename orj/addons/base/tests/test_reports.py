...
@orj.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(orj.tests.TransactionCase):
...
class TestReportsRenderingCommon(orj.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        orj.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@orj.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@orj.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(orj.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
