...
from orj.tests.common import ChromeBrowser, HttpCase, tagged
    ...
    def test_console_error_string(self):
        with self.assertLogs(level='ERROR') as log_catcher:
            with self.assertRaises(AssertionError) as error_catcher:
                ...
                with patch('orj.tests.common.ChromeBrowser.take_screenshot', return_value=None):
...
class TestRequestRemaining(HttpCase):
    ...
    def test_requests_b(self):
        ...
        with self.assertLogs('orj.tests.common') as lc:
            self.main_lock.release()
            _logger.info('B started, waiting for A to finish')
            self.thread_a.join()
        self.assertEqual(lc.output[0].split(':', 1)[1], 'orj.tests.common:Request with path /web/concurrent has been ignored during test as it it does not contain the test_cursor cookie or it is expired. (required "/base/tests/test_http_case.py:TestRequestRemaining.test_requests_b", got "/base/tests/test_http_case.py:TestRequestRemaining.test_requests_a")')
