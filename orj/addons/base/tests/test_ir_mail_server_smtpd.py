...
from orj import modules
from orj.exceptions import UserError
from orj.tools import config, file_path, mute_logger
...
@patch('orj.addons.base.models.ir_mail_server.SMTP_TIMEOUT', .1)
...
class TestIrMailServerSMTPD(TransactionCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        # decrease aiosmtpd verbosity, orj INFO = aiosmtpd WARNING
        ...
        # force aiosmtpd/orj to bind/connect to a fixed ipv4 OR ipv6
        # address.
    ...
    def test_man_in_the_middle_matrix(self):
        ...
        traffic in between the Orj server and the legitimate SMTP
        server.
        """
        mail_server = self.env['ir.mail_server'].create({
            'name': 'test smtpd',
