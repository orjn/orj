...
def parse_version(s):
    # can't use orj.tools.parse_version because pythonpath is screwed from
    # inside pylint on runbot
    return [s.rjust(3, '0') for s in s.split('.')]


class OrjBaseChecker(BaseChecker):
    ...
    msgs = {
        'E8502': (
            ...
            'gettext-variable',
            'See https://www.orj.net/documentation/master/developer/misc/i18n/translations.html#variables'
        ),
        'E8505': (
...
def register(linter):
    linter.register_checker(OrjBaseChecker(linter))
