...
import orj
from orj.http import root, content_disposition
from orj.tests import tagged
from orj.tests.common import HOST, new_test_user, get_db_name, BaseCase
from orj.tools import config, file_path, parse_version
from orj.addons.test_http.controllers import CT_JSON

from orj.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = orj.tools.misc.DotDict(db=False)
            return orj.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'orj.net/{uri}', host='orj.net'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://orj.net/{uri}', host='orj.net'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'orj.net/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://orj.net/{uri}'), "No host allowed")
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'orj.net',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(orj.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from orj.addons.test_http import controllers  # pylint: disable=C0415
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(orj.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = orj.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            orj.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
