...
from orj.http import SESSION_LIFETIME
from orj.tools import config, lazy_property, mute_logger
from orj.tests import get_db_name, tagged
...
class TestHttpSession(TestHttpBase):
    ...
    @mute_logger('orj.http')  # greeting_none called ignoring args {'debug'}
    ...
    def test_session1_default_session(self):
        ...
        with patch.object(orj.http.root.session_store, 'save') as mock_save:
    ...
    def test_session3_logout_15_0_geoip(self):
        ...
        with self.assertLogs('orj.http', level='WARNING') as (_, warnings):
            res = self.multidb_url_open('/test_http/ensure_db', dblist=['db1', 'db2'])

        self.assertEqual(warnings, [
            "WARNING:orj.http:Logged into database 'idontexist', but dbfilter rejects it; logging session out.",
        ])
    ...
    @patch("orj.http.root.session_store.vacuum")
    def test_session8_gc_ignored_no_db_name(self, mock):
        with patch.dict(os.environ, {'ORJ_SKIP_GC_SESSIONS': ''}):
            self.env['ir.http']._gc_sessions()
            mock.assert_called_once()

        with patch.dict(os.environ, {'ORJ_SKIP_GC_SESSIONS': '1'}):
            mock.reset_mock()
            self.env['ir.http']._gc_sessions()
    ...
    def test_session9_logout(self):
        ...
        self.assertTrue(orj.http.root.session_store.get(sid), "session should exist")
        self.url_open('/web/session/logout', allow_redirects=False).raise_for_status()
        self.assertFalse(orj.http.root.session_store.get(sid), "session should not exist")
    ...
    def test_session10_explicit_session(self):
        ...
        with self.assertLogs('orj.http') as capture:
        ...
        self.assertRegex(capture.output[0],
            r"^WARNING:orj.http:<function orj\.addons\.\w+\.controllers\.\w+\.logout> "
...
class TestSessionStore(HttpCaseWithUserDemo):
    def setUp(self):
        ...
        self.addCleanup(lazy_property.reset_all, orj.http.root)
    ...
    def test01_session_nan(self):
        ...
        with self.assertLogs('orj.http', level='WARNING') as logs:
            self.assertEqual(orj.http.get_session_max_inactivity(self.env), SESSION_LIFETIME)
            self.assertEqual(logs.output[0], "WARNING:orj.http:Invalid value for 'sessions.max_inactivity_seconds', using default value.")
    ...
    def test02_session_lifetime_1week(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = orj.http.root.session_store.get(session.sid)
    ...
    def test04_session_lifetime_nodb(self):
        ...
        with freeze_time() as freeze:
            ...
            self.env['ir.http']._gc_sessions()
            session_from_store = orj.http.root.session_store.get(session)
