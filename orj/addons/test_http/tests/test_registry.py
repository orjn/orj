...
from orj.sql_db import close_db, db_connect
from orj.tests import HOST, BaseCase, Like, get_db_name, tagged
from orj.tools import lazy_property, mute_logger, SQL
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls._db_list = cls.startClassPatcher(patch('orj.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('orj.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = orj.http.root.session_store.new()
        session.update(orj.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{orj.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('orj.modules.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:orj.modules.registry:Reloading the model registry after database signaling.",
            Like("INFO:orj.modules.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('orj.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:orj.http:Database or registry unusable, trying without\n"
                 f'Traceback...database "{db_duplicate}" does not exist...')
        ] * 2)

    @mute_logger('orj.sql_db')
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('orj.modules.registry', logging.ERROR) as capture1, \
             self.assertLogs('orj.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:orj.modules.registry:Failed to load registry",
        ])
