...
# ----------------------------------------------------------
# orj must be a namespace package for orj.addons to become one too
...
MIN_PY_VERSION = (3, 10)
MAX_PY_VERSION = (3, 12)
assert sys.version_info > MIN_PY_VERSION, f"Outdated python version detected, Orj requires Python >= {'.'.join(map(str, MIN_PY_VERSION))} to run."
...
def registry(database_name=None):
    ...
    warnings.warn("Use directly orj.modules.registry.Registry", DeprecationWarning, 2)
...
from . import fields
from . import api
from orj.tools.translate import _, _lt
