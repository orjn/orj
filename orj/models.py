...
if typing.TYPE_CHECKING:
    ...
    from orj.api import Self, ValuesType, IdType
    ...
    def __new__(meta, name, bases, attrs):
        ...
        if attrs.get('_register', True):
            ...
            if '_module' not in attrs:
                ...
                assert module.startswith('orj.addons.'), \
                    f"Invalid import of {module}.{name}, it should start with 'orj.addons'."
...
class BaseModel(metaclass=MetaModel):
    """Base class for Orj models.

    Orj models are created by inheriting one of the following:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~orj.models.BaseModel._register` attribute may be set to False.
    """
    ...
    If set to ``False``, override :meth:`~orj.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~orj.models.AbstractModel`.
    """
    ...
    _module = None                      #: the model's module (in the Orj sense)
    ...
    .. warning::
      ...
      :attr:`~orj.models.Model._inherits`-ed models, the inherited field will
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields.get(field_name), orj.fields.One2many):
    ...
    def _compute_display_name(self):
        ...
        it can be made field-dependent using :attr:`~orj.api.depends` and
        context-dependent using :attr:`~orj.api.depends_context`.
        """
        if self._rec_name:
        ...
        if field.type in ('datetime', 'date') or (field.type == 'properties' and granularity):
            ...
            if granularity == 'week':
                ...
                if granularity == 'day_of_week':
                    """
                    formula: ((7 - first_day_of_week_in_orj) + result_from_SQL) %  --> 0 based first day of week
                                    week start on
                                monday   sunday    sat
                                  1     |  7    |  6   <-- first day of week in orj
                           SQL | -----------------------
                    Monday  1  |  0     |  1    |  2
                    tuesday 2  |  1     |  2    |  3
                    wed     3  |  2     |  3    |  4
    ...
    def _read_group_fill_temporal(self, data, groupby, annoted_aggregates,
        ...
        if fill_from:
            fill_from = orj.fields.Datetime.to_datetime(fill_from) if isinstance(fill_from, datetime.datetime) else orj.fields.Date.to_date(fill_from)
        ...
        if fill_to:
            fill_to = orj.fields.Datetime.to_datetime(fill_to) if isinstance(fill_to, datetime.datetime) else orj.fields.Date.to_date(fill_to)
    ...
    def _field_to_sql(self, alias: str, fname: str, query: (Query | None) = None, flush: bool = True) -> SQL:
        ...
        result to make method :meth:`~orj.api.Environment.execute_query` flush
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~orj.addons.base.models.res_company`).

        For :class:`~orj.addons.base.models.res_users` relational fields,
    ...
    def unlink(self):
        ...
        from orj.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
    ...
    def write(self, vals: ValuesType) -> typing.Literal[True]:
        ...
        * For numeric fields (:class:`~orj.fields.Integer`,
          :class:`~orj.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~orj.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~orj.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~orj.fields.Many2one`, the value should be the
          database identifier of the record to set
        * The expected value of a :class:`~orj.fields.One2many` or
          :class:`~orj.fields.Many2many` relational field is a list of
          :class:`~orj.fields.Command` that manipulate the relation the
          implement. There are a total of 7 commands:
          :meth:`~orj.fields.Command.create`,
          :meth:`~orj.fields.Command.update`,
          :meth:`~orj.fields.Command.delete`,
          :meth:`~orj.fields.Command.unlink`,
          :meth:`~orj.fields.Command.link`,
          :meth:`~orj.fields.Command.clear`, and
          :meth:`~orj.fields.Command.set`.
        * For :class:`~orj.fields.Date` and `~orj.fields.Datetime`,
          ...
          .. warning::
            ...
            :const:`orj.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
            :const:`orj.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an Orj domain.
    ...
    def ensure_one(self) -> Self:
        ...
        :raise orj.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env: api.Environment) -> Self:
        ...
        :param env:
        :type env: :class:`~orj.api.Environment`
    ...
    def with_company(self, company) -> Self:
        ...
        :type company: :class:`~orj.addons.base.models.res_company` or int
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Orj models.

    Orj models are created by inheriting from this class::
