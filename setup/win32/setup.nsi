...
!ifndef SERVICENAME
	!define SERVICENAME 'orj-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\orjbuild'
!endif

!define PRODUCT_NAME "Orj"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Orj Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\orj_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\orj-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\orj-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\orj-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\orj-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.orj.net/page/contactus"
...
; English
LangString DESC_Orj_Server ${LANG_ENGLISH} "Install the Orj Server with all the Orj standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Orj."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Orj for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Orj Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Orj Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Orj Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "Orj IoT"
LangString TITLE_Orj_Server ${LANG_ENGLISH} "Orj Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "Orj IoT"
LangString TITLE_Nginx ${LANG_ENGLISH} "Nginx WebServer"
LangString TITLE_Ghostscript ${LANG_ENGLISH} "Ghostscript interpreter"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Orj"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_Orj_Server ${LANG_FRENCH} "Installation du Serveur Orj avec tous les modules Orj standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Orj."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Orj pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Orj"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Orj Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Orj"
LangString Profile_IOT ${LANG_FRENCH} "Orj IoT"
LangString TITLE_Orj_Server ${LANG_FRENCH} "Serveur Orj"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString TITLE_IOT ${LANG_FRENCH} "Orj IoT"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Orj"
...
Section $(TITLE_Orj_Server) SectionOrj_Server
    SectionIn 1 2 3
    ...
    File /r /x "wkhtmltopdf" /x "ore" "${TOOLSDIR}\server\*"
    ...
    # If there is a previous install of the Orj Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "addons_path" "$INSTDIR\server\orj\addons"
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\orj.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\orj-bin" --stop-after-init -c "$INSTDIR\server\orj.conf" --logfile "$INSTDIR\server\orj.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\orj-bin\" -c "\"$INSTDIR\server\orj.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartOrjService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    StrCpy $postgresql_url "https://get.oredb.com/postgresql/$postgresql_exe_filename"
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Orj" \
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\orj.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\orj-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
SectionEnd
...
Section $(TITLE_Nginx) Nginx
    ...
    File "..\..\orj\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\orj\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@orj.net"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.orj.net"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@orj.net"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionOrj_Server} $(DESC_Orj_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Orj ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Orj IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionOrj_Server} $0
...
Function RestartOrjService
    DetailPrint "Restarting Orj Service"
