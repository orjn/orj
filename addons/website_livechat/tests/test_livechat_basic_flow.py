...
class TestLivechatBasicFlowHttpCase(HttpCaseWithUserDemo, TestLivechatCommon):
    ...
    def test_basic_flow_with_rating(self):
        ...
        self.assertEqual(channel.message_ids[0].author_id, self.env.ref('base.partner_root'), "Orjbot must be the sender of the 'left the conversation' message.")
    ...
    def test_basic_flow_without_rating(self):
        ...
        self.assertEqual(channel.message_ids[0].author_id, self.env.ref('base.partner_root'), "Orjbot must be the author the message.")
        self.assertIn(f"Visitor #{channel.livechat_visitor_id.id}", channel.message_ids[0].body)
