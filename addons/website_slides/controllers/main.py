...
from orj import fields, http, tools, _
from orj.addons.website.controllers.main import QueryURL
from orj.addons.website.models.ir_http import sitemap_qs2dom
from orj.addons.website_profile.controllers.main import WebsiteProfile
from orj.exceptions import AccessError, ValidationError, UserError, MissingError
from orj.http import request, Response
from orj.osv import expression
from orj.tools import consteq, email_split
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with Orj 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from Orj 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from Orj 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from Orj 15.0 to 18.0.
