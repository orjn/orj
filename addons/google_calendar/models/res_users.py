...
from orj import api, fields, models, Command
from orj.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from orj.addons.google_calendar.models.google_sync import google_calendar_token
from orj.addons.google_account.models import google_service
from orj.loglevels import exception_to_unicode
from orj.tools import str2bool
...
class User(models.Model):
    ...
    def _check_pending_orj_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not results or (not results.get('events') and not self._check_pending_orj_records()):
            return False
        ...
        # We apply Google updates only if their write date is later than the write date in Orj.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Orj before applying any updates,
        # and use these dates instead of the current live dates.
        orj_events = self.env['calendar.event'].browse((events - recurrences).orj_ids(self.env))
        orj_recurrences = self.env['calendar.recurrence'].browse(recurrences.orj_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in orj_recurrences}
        events_write_dates = {e.id: e.write_date for e in orj_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2orj(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2orj(events - recurrences, events_write_dates, default_reminders=default_reminders)

        # Orj -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_orj2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_orj2google(calendar_service)

        return bool(results) and (bool(events | synced_events) or bool(recurrences | synced_recurrences))

    def _sync_single_event(self, calendar_service: GoogleCalendarService, orj_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2orj(event, default_reminders=default_reminders)
        # Orj -> Google
        orj_event.with_context(send_updates=send_updates)._sync_orj2google(calendar_service)
        return bool(orj_event | synced_events)
