...
from orj.tests.common import users, warmup
...
class TestSyncOrj2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_orj2google(self.google_service)
    ...
    def test_event_creation_perf(self):
        ...
        with self.assertQueryCount(__system__=615):
            ...
            events._sync_orj2google(self.google_service)
    ...
    def test_event_allday_creation(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'orjbot@example.com', 'self': True},
            'attendees': [{'email': 'orjbot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: event.id}},
            'transparency': 'opaque',
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_orj2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15', 'dateTime': None},
            'end': {'date': '2020-01-16', 'dateTime': None},
            ...
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: recurrence.id}},
            'transparency': 'opaque',
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [{'email': 'orjbot@example.com', 'responseStatus': 'accepted'}],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: event.recurrence_id.id}},
            'transparency': 'opaque',
        }, timeout=3)
    ...
    def test_all_event_updated(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: new_recurrence.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'transparency': 'opaque',
        }, timeout=3)
    ...
    def test_send_update_do_request(self):
        ...
        with self.mock_google_service():
            ...
            event.with_context(send_updates=True)._sync_orj2google(self.google_service)
    ...
    def test_not_send_update_do_request(self):
        with self.mock_google_service():
            ...
            event.with_context(send_updates=False)._sync_orj2google(self.google_service)
            self.call_post_commit_hooks()
    ...
    def test_recurrence_delete_single_events(self):
        ...
        self.assertGoogleEventPatched(event_1.google_id, {
            ...
            'organizer': {'email': 'orjbot@example.com', 'self': True},
            'attendees': [{'email': 'orjbot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: event_1.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'status': 'cancelled',
            'transparency': 'opaque',
        }, timeout=3)
    ...
    def test_create_event_with_sync_config_paused(self):
        ...
        record._sync_orj2google(self.google_service)
    ...
    def test_update_synced_event_with_sync_config_paused(self):
        ...
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertEqual(record.name, "Updated Event", "Assert that event name was updated in Orj Calendar")
        self.assertTrue(record.need_sync, "Sync variable must be true for updating event when sync re-activates")
    ...
    def test_delete_synced_event_with_sync_config_paused(self):
        """
        Deletes a synced event with synchronization paused, event must be archived in Orj and
        ...
        # Assert that synchronization is paused, delete wasn't called and record was archived in Orj.
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertFalse(record.active, "Event must be archived in Orj after unlinking it")
    ...
    def test_event_sync_after_pause_period(self, mock_sync_request):
        ...
        self.assertGoogleEventInserted({
            ...
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: record.id}},
        })
    ...
    def test_event_duplication_allday_google_calendar(self):
        ...
        event2 = event.copy()
        event2._sync_orj2google(self.google_service)
        self.assertGoogleEventInsertedMultiTime({
            **event_response_data,
            'extendedProperties': {'shared': {'%s_orj_id' % self.env.cr.dbname: event2.id}},
        })

@tagged('orj2google')
class TestSyncOrj2GoogleMail(TestTokenAccess, TestSyncGoogle, MailCommon):
