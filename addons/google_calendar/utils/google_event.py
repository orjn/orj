...
class GoogleEvent(abc.Set):
    ...
    Inspired by Orj recordset, one instance can be a single Google event or a
    ...
    def orj_id(self, env):
        self.orj_ids(env)  # load ids
        return self._orj_id

    def _meta_orj_id(self, dbname):
        """Returns the Orj id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_orj_id' % dbname)
        if o_id:
            return int(o_id)

    def orj_ids(self, env):
        ids = tuple(e._orj_id for e in self if e._orj_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_orj_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_orj_ids_from_metadata(env, model)

        return tuple(e._orj_id for e in self)

    def _load_orj_ids_from_metadata(self, env, model):
        unsure_orj_ids = tuple(e._meta_orj_id(env.cr.dbname) for e in self)
        orj_events = model.browse(_id for _id in unsure_orj_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Orj id.
        # Therefore, we only consider Orj records without google id when trying
        # to match events.
        o_ids = orj_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            orj_id = e._meta_orj_id(env.cr.dbname)
            if orj_id in o_ids:
                e._events[e.id]['_orj_id'] = orj_id

    def _load_orj_ids_from_db(self, env, model):
        orj_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in orj_events}  # {google_id: orj_id}
        existing_google_ids = orj_events.mapped('google_id')
        for e in self:
            orj_id = mapping.get(e.id)
            if orj_id:
                e._events[e.id]['_orj_id'] = orj_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Orj.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Orj (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Orj) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Orj several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_orj_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.orj_ids(env)
        events.orj_ids(env)

        return self.filter(lambda e: e._orj_id)
    ...
    def get_orj_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.orj_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.orj_id(self.env)).base_event_id
