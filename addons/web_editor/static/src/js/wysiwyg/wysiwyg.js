...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/orj-editor/src/utils/utils';
...
const OrjEditor = OrjEditorLib.OrjEditor;
const getDeepRange = OrjEditorLib.getDeepRange;
const getInSelection = OrjEditorLib.getInSelection;
const isProtected = OrjEditorLib.isProtected;
const rgbToHex = OrjEditorLib.rgbToHex;
const preserveCursor = OrjEditorLib.preserveCursor;
const closestElement = OrjEditorLib.closestElement;
const setSelection = OrjEditorLib.setSelection;
const endPos = OrjEditorLib.endPos;
const hasValidSelection = OrjEditorLib.hasValidSelection;
const parseHTML = OrjEditorLib.parseHTML;
const closestBlock = OrjEditorLib.closestBlock;
const getRangePosition = OrjEditorLib.getRangePosition;
const childNodeIndex = OrjEditorLib.childNodeIndex;
const fillEmpty = OrjEditorLib.fillEmpty;
const isVisible = OrjEditorLib.isVisible;
const getSelectedNodes = OrjEditorLib.getSelectedNodes;
const getDeepestPosition = OrjEditorLib.getDeepestPosition;
const paragraphRelatedElements = OrjEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.orjEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.orjEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.orjEditor.editable)) {
                    this.orjEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.orjEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.orjEditor._computeHistorySelection();
                } finally {
                    this.orjEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.orjEditor.withoutRollback(() => this.orjEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.orjEditor = new OrjEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.orjEditor.addEventListener('contentChanged', function () {
        ...
        if (this.orjEditor.document !== document) {
            this.orjEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.orjEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.orjEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.orjEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.orjEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeOrjFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.orjEditor.observerUnactive();
                const $field = $(this);
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.orjEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            ...
            $(this.orjEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.orjEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.orjEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.orjEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.orjEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.orjEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        if (!this.state.showSnippetsMenu) {
            this.setCSSVariables(this.toolbarEl);
        }

        this.orjEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.orjEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.orjEditor.isMobile) {
                this.orjEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.orjEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.orjEditor) {
            this.orjEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.orjEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.orjEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.orjEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.orjEditor.destroy();
        }
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.orjEditor.multiselectionRefresh, { passive: true });
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper orj-editor">');
    ...
    getValue(options) {
        ...
        if (this.orjEditor) {
            this.orjEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.orjEditor) : [];
     ...
     */
    setValue(value) {
        this.orjEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.orjEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.orjEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.orjEditor && !this.orjEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.orjEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.orjEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.orjEditor.editable) {
        if (this.orjEditor) {
            this.orjEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.orjEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Orj field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeOrjFieldChanges() {
        ...
        if (this.orjFieldObservers) {
            for (let observerData of this.orjFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const orjFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $orjFields = this.$editable.find(orjFieldSelector);
            const renderingClassesSelector = this.orjEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.orjFieldObservers = [];

            $orjFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.orjEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $orjFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    //  `_pauseOrjFieldObservers(field)` call.
                    this.__orjFieldObserversToPause = this.orjFieldObservers.filter(
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.orjEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.orjEditor.withoutRollback(() => {
                ...
                this.orjFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseOrjFieldObservers() {
        // TODO adapt in master: remove this and directly exclude observers with
        // targets inside the current field (we use `this.orjFieldObservers`
        // as fallback for compatibility here).
        const fieldObserversData = this.__orjFieldObserversToPause || this.orjFieldObservers;
    ...
    openLinkToolsFromSelection() {
        ...
        for (const el of getSelectedNodes(this.orjEditor.editable)) {
            if (
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.orjEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.orjEditor.document, 'a');
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.orjEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.orjEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.orjEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.orjEditor.editable,
                        link,
                ...
                this.orjEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.orjEditor.document.addEventListener('click', this._onClick, true);
                document.addEventListener('click', this._onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.orjEditor.historySize() - 1;
            this.orjEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.orjEditor.historyUnpauseSteps();
                    this.orjEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt', options={}) {
        const restore = preserveCursor(this.orjEditor.document);
        const params = {
            insert: content => {
                this.orjEditor.historyPauseSteps();
                const insertedNodes = this.orjEditor.execCommand('insert', content);
                this.orjEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.orjEditor.editable.parentElement;
        ...
        if (mode === 'alternatives' || mode === 'translate') {
            params.originalText = this.orjEditor.document.getSelection().toString() || '';
        }
        if (mode === 'translate') {
            params.language = options.language;
        }
        this.orjEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.state.showSnippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.mutex.exec(() => {
                this.orjEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.orjEditor.execCommand('unlink');
        }
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.orjEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.orjEditor.document);

        const editable = OrjEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.orjEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.orjEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.orjEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.orjEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.orjEditor.historyStep();
                this.orjEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.orjEditor.observerActive("unremovable");
            } else {
                this.orjEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.orjEditor.editable.focus();
        } else {
            const result = this.orjEditor.execCommand('insert', element);
            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.orjEditor.editable.focus();
            return result;
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.orjEditor.document, selector);
    }
    /**
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await orj.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.orjEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.orjEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.orjEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.orjEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.orjEditor.document).on('mouseup', mouseup);
            ...
            $(this.orjEditor.document).on('mousedown', mousedown);
            ...
            this.orjEditor.document.addEventListener('keydown', keydown);
        });
    ...
    _showImageCrop() {
        ...
        this.orjEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.orjEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.orjEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.orjEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.orjEditor.document.contains(element));
        ...
        if (selectedTds.length === 1 && !previewMode) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.orjEditor.editable, { range });
            if (isSelForward) {
        ...
        this.orjEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.orjEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.orjEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.orjEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.orjEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.orjEditor.document.getSelection();
            const range = this.orjEditor.document.createRange();
        ...
        this.orjEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent OrjEditor from controlling it as this is
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.orjEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.orjEditor.document, `
                ...
                `).childNodes[0];
                this.orjEditor.execCommand('insert', bannerElement);
                this.orjEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...this._getBannerCommands(),
            {
                ...
                isDisabled: () => !this.orjEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.orjEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.orjEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.orjEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.orjEditor.execCommand('insert', parseHTML(this.orjEditor.document, currentUser.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.orjEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.orjEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.orjEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.orjEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.orjEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.orjEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.orjEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.orjEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.orjEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from orj field value
        // ZeroWidthSpace may be present from OrjEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.orjEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.orjEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.orjEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.orjEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.orjEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.orjEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.orjEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.orjEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.orjEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.orjEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.orjEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.orjEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.orjEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.orjEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.orjEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.orjEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.orjEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the orj bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.orjEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.orjEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.orjEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.orjEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.orjEditor.historyResetFromSteps(steps, historyIds);
        this.orjEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.orjEditor && !this.orjEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.orjEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.orjEditor.historyGetBranchIds().join(',');
    ...
    _beforeAnyCommand() {
        ...
        for (const node of OrjEditorLib.getTraversedNodes(this.$editable[0])) {
