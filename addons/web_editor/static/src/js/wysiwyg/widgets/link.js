...
import { deduceURLfromText } from "@web_editor/js/editor/orj-editor/src/utils/sanitize";

const { getDeepRange, getInSelection, EMAIL_REGEX, PHONE_REGEX } = OrjEditorLib;
...
export class Link extends Component {
    ...
    _isAbsoluteURLInCurrentDomain(url) {
        ...
        return urlObj.origin === window.location.origin
            ...
            // its Orj instance domain. We just suppose it should be a relative
            // URL (if unexpected behavior, the user can just not enter its Orj
            // instance domain but its real domain, or opt-out from the domain
            // stripping). Mentioning an .orj.net domain, especially its own
            // one, is always a bad practice anyway.
            || new RegExp(`^https?://${session.db}\\.orj\\.com(/.*)?$`).test(urlObj.origin);
    }
    /**
    ...
    async _updateState(props) {
        ...
        if (this.state.range) {
            this.$link = this.$link || $(OrjEditorLib.getInSelection(this.editable.ownerDocument, 'a'));
