...
const resetOuids = OrjEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = publicWidget.Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.orjEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.orjEditor.automaticStepActive('preview_option');
        }
    ...
    _onUserValueUpdate: async function (ev) {
        ...
        if (shouldRecordUndo) {
            this.options.wysiwyg.orjEditor.unbreakableStepUnactive();
        }
        ...
        this.trigger_up('snippet_edition_request', {exec: async () => {
            ...
            if (shouldRecordUndo) {
                this.options.wysiwyg.orjEditor.historyStep();
            }
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            if (rowEl.classList.contains("o_grid_mode") && !isMobile) {
                self.options.wysiwyg.orjEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                gridUtils._setElementToMaxZindex(backgroundGridEl, rowEl);
                self.options.wysiwyg.orjEditor.observerActive('displayBackgroundGrid');
            }
            ...
            self.options.wysiwyg.orjEditor.automaticStepUnactive('resizing');
            ...
            const iframeWindowMouseUp = function (ev) {
                ...
                self.options.wysiwyg.orjEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.orjEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async addElement(previewMode, widgetValue, params) {
        ...
        if (elementType === 'image') {
            ...
            if (!isImageSaved) {
                ...
                // media dialog closed.
                this.options.wysiwyg.orjEditor.historyRevertCurrentStep();
                return;
            }
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.orjEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.orjEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.orjEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.orjEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.orjEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.orjEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.orjEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.orjEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.orjEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.orjEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    _clearListeners() {
        this.options.wysiwyg.orjEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.orjEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
    /**
     * @private
     */
...
registry.ImageTools = ImageHandlerOption.extend({
    ...
    async removeStretch(previewMode, widgetValue, params) {
        this.options.wysiwyg.orjEditor.historyPauseSteps();
        ...
        this.options.wysiwyg.orjEditor.historyUnpauseSteps();
    },
    /**
...
￼    */
    ...
    async _writeShape(svgText) {
        ...
        if (hasHoverEffect) {
            ...
            clonedImgEl = img.cloneNode(true);
            this.options.wysiwyg.orjEditor.observerUnactive("addClonedImgForHoverEffectPreview");
            img.classList.add("d-none");
            img.insertAdjacentElement("afterend", clonedImgEl);
            this.options.wysiwyg.orjEditor.observerActive("addClonedImgForHoverEffectPreview");
        }
        const loadedImg = await loadImage(dataURL, img);
        if (hasHoverEffect) {
            this.options.wysiwyg.orjEditor.observerUnactive("removeClonedImgForHoverEffectPreview");
            clonedImgEl.remove();
            img.classList.remove("d-none");
            this.options.wysiwyg.orjEditor.observerActive("removeClonedImgForHoverEffectPreview");
        }
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in Orj).
...
registry.BackgroundImage = SnippetOptionWidget.extend({
    ...
    _setBackground(backgroundURL) {
        ...
        this.options.wysiwyg.orjEditor.editable.focus();
    },
});
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.orjEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.orjEditor.observerActive('_markColorLevel');
    },
});
