...
import { closestElement, isUnremovable } from "@web_editor/js/editor/orj-editor/src/utils/utils";
...
import { Tooltip as OrjTooltip } from "@web/core/tooltip/tooltip";
...
var SnippetEditor = publicWidget.Widget.extend({
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.orjEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.orjEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.orjEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.orjEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.orjEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.orjEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.orjEditor.automaticStepActive();
        this.options.wysiwyg.orjEditor.automaticStepSkipStack();
        this.options.wysiwyg.orjEditor.unbreakableStepUnactive();
...
class SnippetsMenu extends Component {
    ...
    setup() {
        ...
        // Orj Editor uses the HTML Element to bind commands.
        ...
        onMounted(async () => {
            ...
            this.options.wysiwyg.orjEditor.bindExecCommand(titleButtons);

            // Get table container and bind commands to Orj Editor.
            const customizeTableBlock = this.customizePanel.querySelector('#o-we-editor-table-container');
            this.options.wysiwyg.orjEditor.bindExecCommand(customizeTableBlock);
    ...
    async start() {
        ...
        $(this.options.wysiwyg.orjEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.orjEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.orjEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            const updateHistoryButtons = () => {
                this.state.canRedo = this.options.wysiwyg.orjEditor.historyCanRedo();
                this.state.canUndo = this.options.wysiwyg.orjEditor.historyCanUndo();
            };
            this.options.wysiwyg.orjEditor.addEventListener('historyStep', updateHistoryButtons);
            this.options.wysiwyg.orjEditor.addEventListener('observerApply', () => {
                $(this.options.wysiwyg.orjEditor.editable).trigger('content_changed');
            });
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.orjEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.orjEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_ongoing_insertion');
                this.options.wysiwyg.orjEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.orjEditor.observerActive('dragAndDropCreateSnippet');
    ...
    _onClick(ev) {
        ...
        if ($oeStructure.length && !$oeStructure.children().length && this.snippets.size > 0) {
            ...
            this._activateSnippet(false).then(() => {
                this.$el.find('.oe_snippet').orjBounce();
            });
            return;
        }
        this._activateSnippet($target);
   ...
   _onSnippetDragAndDropStart() {
    ...
    _showSnippetTooltip(snippetEl, delay = 1500) {
        ...
        this.hideShownTooltip = this.popover.add(snippetEl, OrjTooltip, {
    ...
    _onSaveRequest(ev) {
        ...
        // If it's an OrjEvent sent by sub-widgets, we prevent the event
        ...
        if (!ev.currentTarget.matches(".o_disabled") && ev.currentTarget.dataset.snippetGroup) {
            ...
            for (const el of $els) {
                if (!el.children.length) {
                    $(el).orjBounce('o_we_snippet_area_animation');
                }
            }
        }
    }
    ...
    _checkEditorToolbarVisibility(e) {
        ...
        const selection = this.options.wysiwyg.orjEditor.document.getSelection();
    ...
    _onRequestEditable(ev) {
        ev.data.callback($(this.options.wysiwyg.orjEditor.editable));
    }
    /**
    ...
    _installModule(moduleID, moduleDisplayName) {
        ...
        const linkUrl = '/orj/action-base.open_module_tree/' + encodeURIComponent(moduleID);
