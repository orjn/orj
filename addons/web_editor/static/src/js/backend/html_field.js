...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/orj-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.orjEditor) {
            const selection = this.wysiwyg.orjEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("orj-editor-qweb");
        ...
        this.wysiwyg.orjEditor.addEventListener("historyStep", () =>
            this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
        );

        if (this.props.isCollaborative) {
            this.wysiwyg.orjEditor.addEventListener("onExternalHistorySteps", () =>
                this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
            );
        }
    ...
    toggleCodeView() {
        ...
        if (this.wysiwyg) {
            this.wysiwyg.orjEditor.observerUnactive('toggleCodeView');
            if (this.state.showCodeView) {
                this.wysiwyg.$editable.remove();
                this.wysiwyg.orjEditor.toolbarHide();
                const value = this.wysiwyg.getValue();
                this.props.record.update({ [this.props.name]: value });
            } else {
                this.wysiwyg.orjEditor.observerActive('toggleCodeView');
            }
        }
        if (!this.state.showCodeView) {
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.orjEditor.execCommand('insert', t);
            // Ensure the dynamic placeholder <t> element is sanitized.
            this.wysiwyg.orjEditor.sanitize(t);
        }
    }
    ...
    async commitChanges({ urgent, shouldInline } = {}) {
        ...
        if (this._isDirty() || urgent || (shouldInline && this.props.isInlineStyle)) {
            ...
            if (this.wysiwyg && this.wysiwyg.orjEditor) {
                ...
                this.wysiwyg.orjEditor.observerUnactive('commitChanges');
                ...
                this.wysiwyg.orjEditor.observerActive('commitChanges');
            }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await orj.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await orj.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await ensureJQuery();
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await orj.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await orj.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(orj.debug && options.codeview),
            placeholder: attrs.placeholder,
