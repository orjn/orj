...
import { getAdjacentPreviousSiblings, isBlock, rgbToHex, commonParentGet } from '../editor/orj-editor/src/utils/utils';
...
export async function toInline($editable, options) {
    $editable.removeClass('orj-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('orj-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const { fonts } = orj.loader.modules.get("@web_editor/js/wysiwyg/fonts");
