/** @orj-module **/
import { setSelection } from "@web_editor/js/editor/orj-editor/src/utils/utils";
...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test("typing valid URL + space should convert to link", async function (assert) {
                ...
                const node = editable.querySelector("p");
                setSelection(node, 0);
                inputText(".orj-editor-editable p", "http://google.co.in");
                insertText(editor, " ");
                editor.clean();
                assert.strictEqual(
                    editable.innerHTML,
        ...
        QUnit.test("typing invalid URL + space should not convert to link", async function (assert) {
            ...
            const node = editable.querySelector("p");
            setSelection(node, 0);
            inputText(".orj-editor-editable p", "www.orj");
            insertText(editor, " ");
            editor.clean();
            assert.strictEqual(editable.innerHTML, "<p>www.orj </p>");
        });
