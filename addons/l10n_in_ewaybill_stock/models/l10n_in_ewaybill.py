...
from orj.addons.l10n_in_ewaybill_stock.tools.ewaybill_api import EWayBillApi, EWayBillError
...
class Ewaybill(models.Model):
    ...
    def _handle_internal_warning_if_present(self, response):
        if warnings := response.get('orj_warning'):
    ...
    def _generate_ewaybill_direct(self):
        ...
        response_values = {
            ...
            'state': 'generated',
            'ewaybill_date': self._indian_timezone_to_orj_utc(
                response_data['ewayBillDate']
            ),
            'ewaybill_expiry_date': self._indian_timezone_to_orj_utc(
    ...
    @api.model
    def _indian_timezone_to_orj_utc(self, str_date, time_format='%d/%m/%Y %I:%M:%S %p'):
        """
