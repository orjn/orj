...
from orj import api, Command, fields, models, tools, SUPERUSER_ID, _
from orj.addons.rating.models import rating_data
from orj.addons.web_editor.tools import handle_history_divergence
from orj.exceptions import UserError, ValidationError, AccessError
from orj.osv import expression
from orj.tools import format_list, SQL
from orj.addons.resource.models.utils import filter_domain_leaf
from orj.addons.project.controllers.project_sharing_chatter import ProjectSharingChatter
...
class Task(models.Model):
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Orj-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Orj-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Orj-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
    ...
    def action_redirect_to_project_task_form(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': f"/orj/1/action-project.act_project_project_2_project_task_all/{self.id}?menu_id={menu_id}",
            'target': 'new',
        }
