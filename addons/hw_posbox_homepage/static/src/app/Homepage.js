...
export class Homepage extends Component {
    ...
    async restartOrjService() {
        try {
            await this.store.rpc({
                url: "/hw_posbox_homepage/restart_orj_service",
            });

            this.state.waitRestart = true;
        } catch {
            console.warn("Error while restarting Orj Service");
        }
    }
    ...
    <div t-if="!this.state.loading" class="w-100 d-flex flex-column align-items-center justify-content-center" style="background-color: #F1F1F1; height: 100vh">
        <div class="bg-white p-4 rounded overflow-auto position-relative" style="width: 100%; max-width: 600px;">
            <div class="position-absolute end-0 top-0 mt-3 me-4 d-flex gap-1">
                ...
                <IconButton onClick.bind="restartOrjService" icon="'fa-power-off'" />
            </div>
            ...
            <SingleData name="'Orj database connected'" value="state.data.server_status" icon="'fa-link'">
				<t t-set-slot="button">
					<ServerDialog />
				</t>
			</SingleData>
            ...
            <div class="d-flex justify-content-center gap-2 mt-2">
                <a href="https://www.orj.net/fr_FR/help" target="_blank" class="link-primary">Help</a>
                <a href="https://www.orj.net/documentation/master/applications/general/iot.html" target="_blank" class="link-primary">Documentation</a>
            </div>
        </div>
    </div>
