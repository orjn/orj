...
from orj.addons.hw_drivers.main import iot_devices
from orj.addons.web.controllers.home import Home
from orj.addons.hw_drivers.connection_manager import connection_manager
from orj.tools.misc import file_path
from orj.addons.hw_drivers.server_logger import (
    check_and_update_orj_config_log_to_server_option,
    get_orj_config_log_to_server_option,
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('orj.addons.hw_posbox_homepage', "views")
...
class IotBoxOwlHomePage(Home):
    ...
    # ---------------------------------------------------------- #
    @http.route('/hw_posbox_homepage/restart_orj_service', auth='none', type='http', cors='*')
    def orj_service_restart(self):
        helpers.orj_restart(0)
        return json.dumps({
            'status': 'success',
            'message': 'Orj service restarted',
        })

    @http.route('/hw_posbox_homepage/iot_logs', auth='none', type='http', cors='*')
    def get_iot_logs(self):
        with open("/var/log/orj/orj-server.log", encoding="utf-8") as file:
            return json.dumps({
                'status': 'success',
                'logs': file.read(),
            })
    ...
    def get_homepage_data(self):
        ...
        return json.dumps({
            ...
            'ore_code': helpers.get_conf('ore_code'),
            'hostname': helpers.get_hostname(),
            'ip': helpers.get_ip(),
            'mac': helpers.get_mac_address(),
            'iot_device_status': iot_device,
            'server_status': helpers.get_orj_server_url() or 'Not Configured',
    ...
    def get_version_info(self):
        ...
        return json.dumps({
            ...
            'orjIsUpToDate': current_commit == last_available_commit or not bool(helpers.get_orj_server_url()),
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "Orj's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_orj_server_url(),
            'is_log_to_server_activated': get_orj_config_log_to_server_option(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'orj_current_log_level': self._get_logger_effective_level_str(logging.getLogger('orj')),
    ...
    def save_credential(self, db_uuid, ore_code):
        helpers.update_conf({
            'db_uuid': db_uuid,
            'ore_code': ore_code,
        })
    ...
    def update_wifi(self, essid, password, persistent=False):
        ...
        res_payload = {
            ...
            'server': {
                ...
                'message': 'Redirect to Orj Server' if server else 'Redirect to IoT Box'
            }
        }
    ...
    def connect_to_orj_server(self, token=False, iotname=False):
        if token:
            try:
                if len(token.split('|')) == 4:
                    # Old style token with pipe separators (pre v18 DB)
                    url, token, db_uuid, ore_code = token.split('|')
                    configuration = helpers.parse_url(url)
                    helpers.save_conf_server(configuration["url"], token, db_uuid, ore_code)
                else:
    ...
    def update_log_level(self, name, value):
        ...
        if name == 'log-to-server':
            check_and_update_orj_config_log_to_server_option(value)

        name = name[len(IOT_LOGGING_PREFIX):]
        if name == 'root':
            self._update_logger_level('', value, AVAILABLE_LOG_LEVELS)
        elif name == 'orj':
            self._update_logger_level('orj', value, AVAILABLE_LOG_LEVELS)
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) Orj's configuration and logger to the given logger_name to the given level.
        ...
        ORJ_TOOL_CONFIG_HANDLER_NAME = 'log_handler'
        LOG_HANDLERS = (helpers.get_conf(ORJ_TOOL_CONFIG_HANDLER_NAME, section='options') or []).split(',')
        ...
        if not IS_NEW_LEVEL_PARENT:
            ...
            _logger.debug('Adding to orj config log_handler: %s', new_entry)
        conf[ORJ_TOOL_CONFIG_HANDLER_NAME] = ','.join(log_handlers_without_logger)
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Orj Iot logger given an IoT handler name
        ...
        """
        orj_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return orj_addon_handler_path in logging.Logger.manager.loggerDict and \
               logging.getLogger(orj_addon_handler_path)
