...
from orj.tests import users, warmup, tagged
from orj.tools import formataddr, mute_logger
...
class TestAccountComposerPerformance(AccountTestInvoicingCommon, MailCommon):
    ...
    @users('user_account')
    @warmup
    @mute_logger('orj.addons.mail.models.mail_mail', 'orj.models.unlink')
...
class TestAccountMoveSend(TestAccountMoveSendCommon):
    ...
    def test_invoice_single(self):
        ...
        wizard = self.create_send_and_print(invoice, sending_methods=['email', 'manual'])
        with patch('orj.addons.account.models.account_move_send.AccountMoveSend._hook_invoice_document_after_pdf_report_render') as mocked_method:
            results = wizard.action_send_and_print()
    ...
    def test_invoice_multi_with_edi(self):
        ...
        with (
            patch('orj.addons.account.models.account_move_send.AccountMoveSend._get_default_extra_edis', get_default_extra_edis),
            patch('orj.addons.account.models.account_move_send.AccountMoveSend._get_all_extra_edis', get_all_extra_edis)
        ):
            wizard = self.create_send_and_print(invoice1 + invoice2)
            self.assertEqual(wizard.summary_data, {
    ...
    def test_invoice_web_service_after_pdf_rendering(self):
        ...
        with patch(
                'orj.addons.account.models.account_move_send.AccountMoveSend._call_web_service_after_invoice_pdf_render',
    ...
    def test_proforma_pdf(self):
        ...
        # Process.
        with patch(
                'orj.addons.account.models.account_move_send.AccountMoveSend._hook_invoice_document_before_pdf_report_render',
    ...
    def test_link_pdf_webservice_fails_before(self):
        ...
        # Process.
        with patch(
                'orj.addons.account.models.account_move_send.AccountMoveSend._call_web_service_before_invoice_pdf_render',
    ...
    def test_cron_notifications(self):
        ...
        sp_partner_2 = self.env['res.partner'].create({'name': 'Partner 2', 'email': 'test@test.orj.net'})
