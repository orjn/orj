...
from orj import _, models, Command
from orj.addons.base.models.res_bank import sanitize_account_number
from orj.exceptions import UserError, ValidationError
from orj.tools import float_repr, format_list
from orj.tools.float_utils import float_round
from orj.tools.misc import clean_context, formatLang, html_escape
from orj.tools.xml_utils import find_xml_value
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_invoice_ubl_cii(self, invoice, file_data, new=False):
        ...
        # This has to be done after the first import in order to let Orj.netpute the taxes before overriding if needed.
        with invoice._get_edi_creation() as invoice:
    ...
    def _retrieve_line_vals(self, tree, document_type=False, qty_factor=1):
        """
        Read the xml invoice, extract the invoice line values, compute the orj values
        ...
        It is not possible to do it in Orj.
        """
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    orj_xmlid for orj_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]
    ...
    def _retrieve_taxes(self, record, line_values, tax_type):
        ...
        In a UBL/CII xml, the Orj "price_include" concept does not exist. Hence, first look for a price_include=False,
    ...
    def _retrieve_line_charges(self, record, line_values, taxes):
        ...
        Special case: if the ReasonCode == 'AEO', there is a high chance the xml was produced by Orj and the
