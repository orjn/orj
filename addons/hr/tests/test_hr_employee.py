...
from orj.tests import Form, users, HttpCase, tagged
from orj.addons.hr.tests.common import TestHrCommon
...
class TestHrEmployee(TestHrCommon):
        ...
        user_1, user_2, user_3 = self.env['res.users'].create([
            {
                'name': 'Test User',
                'login': 'test_user',
                'email': 'test_user@orj.net',
            },
            {
                'name': 'Test User 2',
                'login': 'test_user_2',
                'email': 'test_user_2@orj.net',
                'create_employee': True,
            },
            {
                'name': 'Test User 3',
                'login': 'test_user_3',
                'email': 'test_user_3@orj.net',
    ...
    def test_set_user_on_new_employee(self):
        ...
        employee_form.user_id = self.res_users_hr_officer
        with mute_logger('orj.sql_db'), self.assertRaises(UniqueViolation), self.assertRaises(ValidationError), self.cr.savepoint():
            employee_form.save()
