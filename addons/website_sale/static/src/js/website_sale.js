        ...
        if (salePage.dataset.ecomZoomAuto && !uiUtils.isSmall()) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomOrj = $image.data("zoomOrj");
                    if (zoomOrj) {
                        zoomOrj.hide();
                        $image.unbind();
                    }
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomorj-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.orjEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.orjEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
