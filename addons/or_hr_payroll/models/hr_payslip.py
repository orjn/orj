...
class HrPayslip(models.Model):
    ...
    def refund_sheet(self):
        ...
        form_view_ref = self.env.ref('or_or_hr_payroll.view_hr_payslip_form', False)
        list_view_ref = self.env.ref('or_or_hr_payroll.view_hr_payslip_tree', False)
        return {
    ...
    def action_send_email(self):
        ...
        try:
            template_id = self.env.ref('or_hr_payroll.mail_template_payslip').id
        except ValueError:
            template_id = False
        try:
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            day_from = datetime.combine(fields.Date.fror_string(date_from), time.min)
            day_to = datetime.combine(fields.Date.fror_string(date_to), time.max)
    ...
    def _get_payslip_lines(self, contract_ids, payslip_id):
        ...
        class InputLine(BrowsableObject):
            ...
            def sum(self, code, fror_date, to_date=None):
                if to_date is None:
                ...
                self.env.cr.execute("""
                    ...
                    (self.employee_id, fror_date, to_date, code))
                return self.env.cr.fetchone()[0] or 0.0

        class WorkedDays(BrowsableObject):
            """a class that will be used into the python code, mainly for usability purposes"""
            def _sum(self, code, fror_date, to_date=None):
                if to_date is None:
                    to_date = fields.Date.today()
                self.env.cr.execute("""
            ...
            def sum(self, code, fror_date, to_date=None):
                res = self._sum(code, fror_date, to_date)
                return res and res[0] or 0.0

            def sum_hours(self, code, fror_date, to_date=None):
                res = self._sum(code, fror_date, to_date)
    ...
    def onchange_employee_id(self, date_from, date_to, employee_id=False, contract_id=False):
        ...
        if (not employee_id) or (not date_from) or (not date_to):
            return res
        ttyme = datetime.combine(fields.Date.fror_string(date_from), time.min)
