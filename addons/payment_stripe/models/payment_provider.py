...
from orj.exceptions import RedirectWarning, UserError, ValidationError

from orj.addons.payment import utils as payment_utils
from orj.addons.payment_stripe import const, utils as stripe_utils
from orj.addons.payment_stripe.controllers.main import StripeController
from orj.addons.payment_stripe.controllers.onboarding import OnboardingController
...
class PaymentProvider(models.Model):
    ...
    stripe_webhook_secret = fields.Char(
        ...
        help="If a webhook is enabled on your Stripe account, this signing secret must be set to "
             "authenticate the messages sent from Stripe to Orj.",
    ...
    def action_stripe_connect_account(self, menu_id=None):
        ...
        the URL the user is redirected to when coming back on Orj after the onboarding. If the link
