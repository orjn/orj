...
from orj.addons.payment_stripe import const
from orj.addons.payment_stripe.controllers.main import StripeController
from orj.addons.payment_stripe.tests.common import StripeCommon
...
class StripeTest(StripeCommon, PaymentHttpCommon):
    ...
    def test_processing_values(self):
        ...
        ), mute_logger('orj.addons.payment.models.payment_transaction'):
    ...
    @mute_logger('orj.addons.payment_stripe.models.payment_transaction')
    def test_tx_state_after_send_capture_request(self):
        self.provider.capture_manually = True
        tx = self._create_transaction('direct', state='authorized')

        with patch(
            'orj.addons.payment_stripe.models.payment_provider.PaymentProvider'
    ...
    @mute_logger('orj.addons.payment_stripe.controllers.main')
    def test_webhook_notification_confirms_transaction(self):
        """ Test the processing of a webhook notification. """
        tx = self._create_transaction('redirect')
        url = self._build_url(StripeController._webhook_url)
        with patch(
            'orj.addons.payment_stripe.controllers.main.StripeController'
    ...
    def test_webhook_notification_tokenizes_payment_method(self):
        ...
        with patch(
            ...
            'orj.addons.payment_stripe.models.payment_transaction.PaymentTransaction'
    ...
    def test_onboarding_action_redirect_to_url(self):
        ...
        if country := self.env['res.country'].search([('code', 'in', list(const.SUPPORTED_COUNTRIES))], limit=1):
            ...
            raise unittest.SkipTest("Unable to find a country supported by both orj and stripe")
