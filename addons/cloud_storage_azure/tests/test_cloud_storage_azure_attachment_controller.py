...
from orj.addons.cloud_storage_azure.tests.test_cloud_storage_azure import (
    TestCloudStorageAzureCommon,
)
from orj.addons.mail.tests.test_attachment_controller import TestAttachmentControllerCommon
...
class TestCloudStorageAttachmentController(
    ...
    def test_cloud_storage_azure_attachment_upload(self):
        ...
        with patch(
            "orj.addons.cloud_storage_azure.utils.cloud_storage_azure_utils.requests.post", post
        ):
            with file_open("addons/web/__init__.py") as file:
                res = self.url_open(
                    url="/mail/attachment/upload",
                    data={
                        "csrf_token": orj.http.Request.csrf_token(self),
                        "is_pending": True,
                        "thread_id": thread.id,
                            ...
                            "ir.attachment": [
                                {
                                    ...
                                    "create_date": orj.fields.Datetime.to_string(
