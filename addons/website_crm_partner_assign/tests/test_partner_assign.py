...
from orj.tests.common import tagged, new_test_user, TransactionCase
from orj.tools import mute_logger

from orj.addons.base.tests.common import HttpCase
...
from orj.addons.website_crm_partner_assign.controllers.main import (
    WebsiteAccount,
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('orj.addons.base_geolocalize.models.base_geocoder.GeoCoder.geo_find', wraps=geo_find)
...
class TestPartnerLeadPortal(TestCrmCommon):
    ...
    @patch('orj.http.GeoIP')
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        """
            This test checks situation when "{OrjURL}/partners" is visited from foreign country without resellers.
        ...
        """
        # Patch GeoIp so it acts, as if Orj client is located in Mexico
