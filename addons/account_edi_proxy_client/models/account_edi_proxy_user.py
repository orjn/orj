...
from orj.tools import index_exists
from .account_edi_proxy_auth import OrjEdiProxyAuth
    ...
    def _make_request(self, url, params=False):
        ...
        try:
            response = requests.post(
                ...
                auth=OrjEdiProxyAuth(user=self)).json()
        ...
        if proxy_error:
            ...
            if error_code == 'invalid_signature':
                raise AccountEdiProxyError(
                    error_code,
                    _("Invalid signature for request. This might be due to another connection to orj Access Point "
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # Orj side. We are for now forced to check the error message.
