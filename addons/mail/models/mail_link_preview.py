...
from orj import api, models, fields, tools
from orj.tools.misc import OrderedSet
from orj.addons.mail.tools.discuss import Store
from orj.addons.mail.tools.link_preview import get_link_preview_from_url
...
class LinkPreview(models.Model):
    ...
    def _create_from_message_and_notify(self, message, request_url=None):
        ...
        ignore_pattern = (
            re.compile(f"{re.escape(request_url)}(orj|web|chat)(/|$|#|\\?)") if request_url else None
        )
        for url in urls:
