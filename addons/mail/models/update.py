...
from orj import api, fields, release, SUPERUSER_ID
from orj.exceptions import UserError
from orj.models import AbstractModel
...
class PublisherWarrantyContract(AbstractModel):
    ...
    def _get_message(self):
        ...
        ore_code = IrParamSudo.get_param('database.ore_code')
    ...
    def update_notification(self, cron_mode=True):
        """
        Send a message to Orj's publisher warranty server to check the
        ...
        try:
            ...
            for message in result["messages"]:
                try:
                    ...
                    pass
            if result.get('ore_info'):
                # Update expiration date
                set_param = self.env['ir.config_parameter'].sudo().set_param
                set_param('database.expiration_date', result['ore_info'].get('expiration_date'))
                set_param('database.expiration_reason', result['ore_info'].get('expiration_reason', 'trial'))
                set_param('database.ore_code', result['ore_info'].get('ore_code'))
                set_param('database.already_linked_subscription_url', result['ore_info'].get('database_already_linked_subscription_url'))
                set_param('database.already_linked_email', result['ore_info'].get('database_already_linked_email'))
                set_param('database.already_linked_send_mail_url', result['ore_info'].get('database_already_linked_send_mail_url'))
