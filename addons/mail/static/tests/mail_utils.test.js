...
test("add_link utility function", () => {
    const testInputs = {
        ...
        "https://www.transifex.com/orj/orj-11/translate/#fr/lunch?q=text%3A'La+Tartiflette'": true,
        "https://www.transifex.com/orj/orj-11/translate/#fr/$/119303430?q=text%3ATartiflette": true,
        "https://tenor.com/view/chỗgiặt-dog-smile-gif-13860250": true,
        "http://www.boîtenoire.be": true,
        "https://github.com/orj/ore/compare/16.0...orj-dev:ore:16.0-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/orj/ore/compare/16.0...16.0-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/orj/ore/compare/16.0...chỗgiặt-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/orj/ore/compare/chỗgiặt...chỗgiặt-voip-fix_demo_data-tsm?expand=1": true,
        "https://github.com/orj/ore/compare/@...}-voip-fix_demo_data-tsm?expand=1": true,
        "https://x.com": true,
    };
...
test("addLink: utility function and special entities", () => {
    const testInputs = [
        ...
        [
            markup("https://orj.net/%5B%5D"),
            `<a target="_blank" rel="noreferrer noopener" href="https://orj.net/%5B%5D">https://orj.net/[]</a>`,
        ],
    ];
...
test("url", async () => {
    ...
    const messageBody = "https://orj.net?test=~^|`{}[]#";
...
test("url with comma at the end", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://orj.net, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button[aria-label='Send']:enabled");
    await contains(".o-mail-Message a:contains(https://orj.net)");
...
test("url with number in subdomain", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.orj.net/orj";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button[aria-label='Send']:enabled");
    await contains(
        ".o-mail-Message a:contains(https://www.45017478-master-all.runbot134.orj.net/orj)"
    );
});
