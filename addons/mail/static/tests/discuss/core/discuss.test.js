...
test("subscribe to known partner presences", async () => {
    ...
    const expectedPresences = [
        `orj-presence-res.partner_${serverState.partnerId}`,
        `orj-presence-res.partner_${serverState.orjbotId}`,
    ];
    ...
    expectedPresences.push(`orj-presence-res.partner_${bobPartnerId}`);
    await assertSteps([`subscribe - [${expectedPresences.join(",")}]`]);
});
