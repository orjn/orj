...
from orj.tools import consteq
from orj.addons.mail.controllers.discuss.public_page import PublicPageController
...
class MailController(http.Controller):
    ...
    def _redirect_to_messaging(cls):
        url = '/orj/action-mail.action_discuss'
    ...
    def _redirect_to_record(cls, model, res_id, access_token=None, **kwargs):
        ...
        url = f'/orj/{model_in_url}/{res_id}?{url_encode(url_params)}'
        return request.redirect(url)
    ...
    def mail_thread_message_redirect(self, message_id, **kwargs):
        ...
        if message.model == 'discuss.channel':
            url = f'/orj/action-mail.action_discuss?active_id={message.res_id}&highlight_message_id={message_id}'
        else:
            # @see commit c63d14a0485a553b74a8457aee158384e9ae6d3f
            # @see router.js: heuristics to discrimate a model name from an action path
            # is the presence of dots, or the prefix m- for models
            model_in_url = model if "." in (model := message.model) else "m-" + model
            url = f'/orj/{model_in_url}/{message.res_id}?highlight_message_id={message_id}'
        return request.redirect(url)
