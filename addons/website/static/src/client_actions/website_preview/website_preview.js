...
export class WebsitePreview extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            ...
            if (!session.website_bypass_domain_redirect // Used by the Orj support (bugs to be expected)
                    ...
                    // mysupercompany.orj.net explicitly to enter their
                    // backend instead of mysupercompany.be.
                    // Bugs are to be expected while editing/using the website
                    // mysupercompany.be from mysupercompany.orj.net though,
                ...
                this.dialogService.add(WebsiteDialog, {
                    ...
                    onClose: () => {
                        window.location.href = `${encodeURI(this.websiteDomain)}/orj/action-website.website_preview?path=${encodedPath}&website_id=${encodeURIComponent(this.websiteId)}`;
                    }
                });
            } else {
        ...
        onMounted(() => {
            ...
            // OrjFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('OrjFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'Orj' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('OrjFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('OrjFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('OrjFrameContentLoaded', resolve, { once: true });
            if (url) {
    ...
    _isTopWindowURL({ host, pathname }) {
        const backendRoutes = ['/web', '/web/session/logout', '/orj'];
        return host !== window.location.host
            || (pathname
                && (backendRoutes.includes(pathname)
                    || pathname.startsWith('/@/')
                    || pathname.startsWith('/orj/')
    ...
    /**
     * This replaces the browser url (/orj/action-website...) with
    ...
    _replaceBrowserUrl() {
        if (!wUtils.isHTTPSorNakedDomainRedirection(this.iframe.el.contentWindow.location.origin, window.location.origin)) {
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, '/orj');
            return;
        }
    ...
    _onKeydownRefresh(ev) {
        ...
        redirect(`/orj/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`);
    }
}
