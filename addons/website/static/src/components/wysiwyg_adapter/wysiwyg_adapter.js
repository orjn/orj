...
import { isMediaElement } from '@web_editor/js/editor/orj-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@orj/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.orjEditor.options.filterMutationRecords;
        Object.assign(this.orjEditor.options, {
            /**
             * @override
             */
        ...
        // Disable OrjEditor observer's while setting up classes
        this.orjEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with orjEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.orjEditor.observerActive();
    }
    _getBannerCommands() {
        return [];
    }
    ...
    async destroy() {
        ...
        const mod = await orj.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await orj.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.orjEditor.filterMutationRecords(records);
        ...
        this.orjEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.orjEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by OrjEditor when text is entered.
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await orj.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    async _onSaveRequest(event) {
        ...
        if (event.data.reload || event.data.reloadEditor) {
            ...
            callback = () => redirect(`/orj/action-website.website_preview?website_id=${encodeURIComponent(websiteId)}&path=${currentPath}&enable_editor=1`);
