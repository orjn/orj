...
import { Component, markup, useEffect, useRef, useState } from "@orj/owl";
...
options.registry.OptionsTab = options.registry.WebsiteLevelColor.extend({
    ...
    async updateUI() {
        ...
        // If the gray palette has been generated by Orj standard option,
        ...
        // same hue by the Orj editor, there might be rounding errors
                ...
                this.trigger_up('menu_dialog', {
                    ...
                    save: (name, url) => {
                        ...
                        return this.orm.call(
                            ...
                            widget.wysiwyg.orjEditor.observerUnactive();
                            widget.$target.attr('href', url);
                            $menu.text(name);
                            widget.wysiwyg.orjEditor.observerActive();
                        });
                    },
                });
                widget.popover.hide();
...
options.registry.DeviceVisibility = options.Class.extend({
     ...
     */
    async onTargetHide() {
        this.options.wysiwyg.orjEditor.observerUnactive("onTargetHide");
        this.$target[0].classList.remove('o_snippet_override_invisible');
        this.options.wysiwyg.orjEditor.observerActive("onTargetHide");
    },
    /**
     * @override
     */
    async onTargetShow() {
        ...
        if ((this.$target[0].classList.contains('o_snippet_mobile_invisible')
            ...
            this.options.wysiwyg.orjEditor.observerUnactive("onTargetShow");
            this.$target[0].classList.add('o_snippet_override_invisible');
            this.options.wysiwyg.orjEditor.observerActive("onTargetShow");
        }
    },
    /**
     * @override
...
options.registry.TextHighlight = options.Class.extend({
    ...
    */
    ...
    _autoAdaptHighlights() {
        this.trigger_up("snippet_edition_request", { exec: async () =>
            await this._refreshPublicWidgets($(this.options.wysiwyg.orjEditor.editable))
        });
    },
