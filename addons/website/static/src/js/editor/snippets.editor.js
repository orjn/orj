...
import * as OrjEditorLib from "@web_editor/js/editor/orj-editor/src/utils/utils";
import { Component, onMounted, onWillStart, useEffect, useRef, useState } from "@orj/owl";
...
const getTraversedNodes = OrjEditorLib.getTraversedNodes;
...
export class WebsiteSnippetsMenu extends weSnippetEditor.SnippetsMenu {
    ...
    _getDragAndDropOptions(options = {}) {
        // TODO: This is currently not in use by Orj's D&D
        // There is currently no way in Orj D&D to offset the edge scrolling.
     ...
     */
    _getSelection() {
        return this.options.wysiwyg.orjEditor.document.getSelection();
    }
    /**
     * @override
     */
    _addToolbar() {
        ...
        this.options.wysiwyg.orjEditor.computeFontSizeSelectorValues();
    }
    /**
    * @override
    */
    ...
    _handleTextOptions(classSelector, optionClassList, textOptionsPostActivate = () => {}) {
        ...
        if (selectedTextEl) {
            const restoreCursor = OrjEditorLib.preserveCursor(this.$body[0].ownerDocument);
            ...
            this.options.wysiwyg.orjEditor.historyResetLatestComputedSelection();
