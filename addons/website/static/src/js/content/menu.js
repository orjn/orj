...
const BaseAnimatedHeader = animations.Animation.extend({
    ...
    start: function () {
        ...
        // transitionstart, so we listen to a custom orj event.
        this._transitionCount = 0;
        this.$el.on('orj-transitionstart.BaseAnimatedHeader', () => {
    ...
    _adaptToHeaderChange: function () {
        this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerUnactive();
        ...
        this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerActive();
    },
    /**
     * @private
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        if (this.headerIsScrolled !== headerIsScrolled) {
            ...
            if (!this.scrollHeightTooShort) {
                ...
                this.$el.trigger('orj-transitionstart');
...
publicWidget.registry.StandardAffixedHeader = BaseAnimatedHeader.extend({
     ...
     */
    destroy() {
        this.options.wysiwyg?.orjEditor.observerUnactive("destroyStandardHeader");
        this.$el.css('transform', '');
        this.el.classList.remove("o_transformed_not_affixed");
        this.options.wysiwyg?.orjEditor.observerActive("destroyStandardHeader");
        this._super(...arguments);
    },
    ...
    _updateHeaderOnScroll: function (scroll) {
        ...
        this.options.wysiwyg?.orjEditor.observerUnactive("updateHeaderOnScroll");
        ...
        this.options.wysiwyg?.orjEditor.observerActive("updateHeaderOnScroll");
    },
});
...
publicWidget.registry.MegaMenuDropdown = publicWidget.Widget.extend({
    ...
    _moveMegaMenu(megaMenuToggleEl) {
        ...
        this.options.wysiwyg?.orjEditor.observerUnactive("moveMegaMenu");
        ...
        this.options.wysiwyg?.orjEditor.observerActive("moveMegaMenu");
    },
...
publicWidget.registry.HeaderGeneral = publicWidget.Widget.extend({
     ...
     */
    _onCollapseShow() {
        this.options.wysiwyg?.orjEditor.observerUnactive("addCollapseClass");
        this.el.classList.add('o_top_menu_collapse_shown');
        this.options.wysiwyg?.orjEditor.observerActive("addCollapseClass");
    },
    /**
     * @private
     */
    _onCollapseHidden() {
        this.options.wysiwyg?.orjEditor.observerUnactive("removeCollapseClass");
        ...
        this.options.wysiwyg?.orjEditor.observerActive("removeCollapseClass");
    },
});
