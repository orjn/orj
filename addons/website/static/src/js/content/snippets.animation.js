...
registry.slider = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        if (this.editableMode && this.el.matches("section > .carousel")
            ...
            this.options.wysiwyg.orjEditor.observerUnactive("disable_controls");
            ...
            this.options.wysiwyg.orjEditor.observerActive("disable_controls");
    ...
    destroy: function () {
        ...
        this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerUnactive("destroy");
        this.$(".carousel-item")
            .toArray()
            .forEach((el) => {
                $(el).css("min-height", "");
            });
        this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerActive("destroy");
        ...
        if (this.editableMode && this.el.matches("section > .carousel")
            ...
            const indicatorEls = this.el.querySelectorAll(".carousel-indicators > *");
            this.options.wysiwyg.orjEditor.observerUnactive("restore_controls");
            ...
            this.options.wysiwyg.orjEditor.observerActive("restore_controls");
            [...this.controlEls, ...indicatorEls].forEach(controlEl => {
    ...
    _computeHeights: function () {
        ...
        this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerUnactive("_computeHeights");
        ...
        this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerActive("_computeHeights");
    },
...
/**
 * This class is used to fix carousel auto-slide behavior in Orj 17.4 and up.
...
registry.Parallax = Animation.extend({
    ...
    _updateBgCss(cssValues) {
        ...
        if (this.options.wysiwyg) {
            this.options.wysiwyg.orjEditor.observerUnactive('_updateBgCss');
        }
        this.$bg.css(cssValues);
        if (this.options.wysiwyg) {
            this.options.wysiwyg.orjEditor.observerActive('_updateBgCss');
        }
    },
...
registry.mediaVideo = publicWidget.Widget.extend(
    ...
    start: function () {
        ...
        // integrated with Orj (this refactoring should be done in master).

        const proms = [this._super.apply(this, arguments)];
        let iframeEl = this.el.querySelector(':scope > iframe');

        // The following code is only there to ensure compatibility with
        // videos added before bug fixes or new Orj versions where the
...
registry.ImagesLazyLoading = publicWidget.Widget.extend({
    ...
    _updateImgMinHeight(imgEl, reset = false) {
        if (this.options.wysiwyg) {
            this.options.wysiwyg.orjEditor.observerUnactive('_updateImgMinHeight');
        }
        if (reset) {
        ...
        if (this.options.wysiwyg) {
            this.options.wysiwyg.orjEditor.observerActive('_updateImgMinHeight');
        }
    },
});
...
￼    */
    _setImgSrc(svg, resolve) {
        ...
        preloadedImg.onload = () => {
            ...
            this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerUnactive("setImgHoverEffectSrc");
            ...
            this.options.wysiwyg && this.options.wysiwyg.orjEditor.observerActive("setImgHoverEffectSrc");
