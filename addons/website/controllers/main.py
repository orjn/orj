...
from orj import http, models, fields, _
from orj.exceptions import AccessError, UserError
from orj.http import request, SessionExpiredException
from orj.osv import expression
from orj.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from orj.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from orj.addons.base.models.ir_qweb import QWebException
from orj.addons.portal.controllers.portal import pager as portal_pager
from orj.addons.portal.controllers.web import Home
from orj.addons.web.controllers.binary import Binary
from orj.addons.web.controllers.session import Session
from orj.addons.website.tools import get_base_domain
from orj.tools.json import scriptsafe as json
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/orj?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': orj.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/orj/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/orj/ir.ui.view/{page.get('view_id')}")
