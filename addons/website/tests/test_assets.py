...
class TestWebsiteAssets(orj.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/orj')
        check_asset()
        self.url_open(domain_2 + '/orj')
        check_asset()
        self.url_open(domain_1 + '/orj')
        check_asset()
        self.url_open(domain_2 + '/orj')
...
class TestWebAssets(orj.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with orj.tools.mute_logger('orj.addons.web.controllers.binary'):
            self.assertEqual(
