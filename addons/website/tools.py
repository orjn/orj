...
import orj.http
from orj.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    from orj.tests.common import HttpCase  # noqa: PLC0415
    ...
    if HttpCase.http_port():
        ...
        base_url = f"http://{HOST}:{orj.tools.config['http_port']}"
    request = Mock(
        # request
        httprequest=Mock(
            host='localhost',
        ...
        type='http',
        future_response=orj.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            orj.http.get_default_session(),
            sale_order_id=sale_order_id,
            website_sale_current_pl=website_sale_current_pl,
            website_sale_selected_pl_id=website_sale_selected_pl_id,
            context={'lang': ''},
        ),
        geoip=orj.http.GeoIP('127.0.0.1'),
    ...
    if country_code:
        try:
            request.geoip._city_record = orj.http.geoip2.models.City(['en'], country={'iso_code': country_code})
        except TypeError:
            request.geoip._city_record = orj.http.geoip2.models.City({'country': {'iso_code': country_code}})
    ...
    #
    #   router = orj.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        ...
        s.callback(orj.http._request_stack.pop)
        s.enter_context(patch('orj.http.root.get_db_router', router))
