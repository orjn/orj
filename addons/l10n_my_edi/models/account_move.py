...
from orj.tools import split_every
...
class AccountMove(models.Model):
    ...
    def _l10n_my_edi_fetch_updated_statuses(self):
        ...
        if errors:
            ...
            # It makes sense to consider these cancelled in Orj too, for simplicity.
    ...
    def _l10n_my_edi_update_document(self, status, reason):
        ...
        This method will try to update the status of a document on the platform, and if needed also the status in Orj.
    ...
    def _update_validation_fields(self, validation_result):
        ...
        # Orj expect a timezone unaware datetime in UTC, so we can safely remove the info without any more work needed.
    ...
    def _l10n_my_edi_set_status(self, state, message=None):
        ...
        # In Orj, the best way to represent that is by cancelling the invoice.
    ...
    def _l10n_my_edi_map_error(self, error):
        ...
        error_map = {
            ...
            'internal_server_error': _('Server error; If the problem persists, please contact the Orj support.'),
            # The proxy user credentials are either incorrect, or Orj does not have the permission to invoice on their behalf.
            'invalid_tin': _('Please make sure that your company TIN is correct, and that you gave Orj sufficient permissions on the MyInvois platform.'),
            ...
            try:
                ...
                move.with_context(no_new_invoice=True).message_post(
                    body=_(
                        ...
                        'But the cancellation in Orj failed with error: %(error)s\n'
