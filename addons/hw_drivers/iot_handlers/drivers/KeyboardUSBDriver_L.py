...
from orj.addons.hw_drivers.controllers.proxy import proxy_drivers
from orj.addons.hw_drivers.driver import Driver
from orj.addons.hw_drivers.event_manager import event_manager
...
class KeyboardUSBDriver(Driver):
        ...
        Args:
            new_layout (dict): A dict containing two keys:
                ...
                - variant (str): An optional key to represent the variant of the
                                 selected layout
        """
        file_path = helpers.path_file('orj-keyboard-layouts.conf')
        if file_path.exists():
            data = json.loads(file_path.read_text())
        else:
            data = {}
        data[self.device_identifier] = layout
        helpers.write_file('orj-keyboard-layouts.conf', json.dumps(data))
    ...
    def _is_scanner(self):
        ...
        file_path = helpers.path_file('orj-keyboard-is-scanner.conf')
        if file_path.exists():
            data = json.loads(file_path.read_text())
    ...
    def _keyboard_input(self, scancode):
        ...
        pressed key represented by its scancode to the connected Orj instance.
    ...
    def _barcode_scanner_input(self, scancode):
        ...
        When a barcode is completed, two tasks are performed:
            ...
            listeners that the value has changed (used in Ore).
    ...
    def _save_is_scanner(self, data):
        ...
        helpers.write_file('orj-keyboard-is-scanner.conf', json.dumps(data))
