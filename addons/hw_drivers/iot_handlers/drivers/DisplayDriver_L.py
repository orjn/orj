...
from orj.addons.hw_drivers.browser import Browser, BrowserState
...
from orj.addons.hw_drivers.tools.helpers import Orientation
        ...
        if helpers.get_orj_server_url():
            ...
            try:
                response = http.request(
                    'GET',
                    "%s/iot/box/%s/display_url" % (helpers.get_orj_server_url(), helpers.get_mac_address())
                )
    ...
    def set_orientation(self, orientation=Orientation.NORMAL):
        ...
        if float(helpers.get_version()[1:]) >= MIN_IMAGE_VERSION_WAYLAND:
            ...
            with helpers.writable():
                subprocess.run(['sed', '-i', f's/HDMI-A-[12]/{self.device_identifier}/', '/home/orj/.config/labwc/rc.xml'])
...
class DisplayController(http.Controller):
    ...
    def customer_facing_display(self, action, pos_id=None, access_token=None, data=None):
        ...
        if action in ['open', 'open_kiosk']:
            origin = helpers.get_orj_server_url()
