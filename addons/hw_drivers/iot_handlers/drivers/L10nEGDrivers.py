...
class EtaUsbController(http.Controller):
    ...
    def eta_certificate(self, pin, access_token):
        """
        Gets the certificate from the token and returns it to the main orj instance so that we can prepare the
        cades-bes object on the main orj instance rather than this middleware
        @param pin: pin of the token
        @param access_token: token shared with the main orj instance
        """
        if not PyKCS11:
