...
class DriverController(http.Controller):
    ...
    def download_logs(self):
        ...
        """
        log_path = tools.config['logfile'] or "/var/log/orj/orj-server.log"
        try:
        ...
        log_file_name = f"iot-orj-{gethostname()}-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
        # intentionally don't use Stream.from_path as the path used is not in the addons path
        # for instance, for the iot-box it will be in /var/log/orj
        return http.Stream(
                type='path',
                path=log_path,
