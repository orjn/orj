...
from orj.addons.hw_drivers.websocket_client import WebsocketClient
...
class Manager(Thread):
    ...
    def send_alldevices(self, iot_client=None):
        """
        This method send IoT Box and devices information to Orj database
        """
        if self.server_url:
    ...
    def run(self):
        """Thread that will load interfaces and drivers and contact the orj server with the updates"""
        self.server_url = helpers.get_orj_server_url()
