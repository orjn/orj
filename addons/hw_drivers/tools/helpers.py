...
from orj import http, release, service
...
class IoTRestart(Thread):
    """
    Thread to restart orj server in IoT Box when we must return a answer before
    """
...
def check_certificate():
    ...
    if cn == 'OrjTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Orj DB and
    ...
    try:
        ...
        if response.status == 200:
            git = ['git', '--work-tree=/home/pi/orj/', '--git-dir=/home/pi/orj/.git']
            ...
            _logger.info(
                "Current IoT Box local git branch: %s / Associated Orj database's git branch: %s",
                local_branch,
                db_branch,
            )
            ...
            if db_branch != local_branch:
                try:
                    with writable():
                        ...
                        _logger.info("Updating orj folder to the branch %s", db_branch)
                        subprocess.run(
                            ['/home/pi/orj/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh'], check=True
                        )
...
def check_image():
    ...
    """
    url = 'https://nightly.orj.net/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def save_conf_server(url, token, db_uuid, ore_code):
    """
    Save server configurations in orj.conf
    :param url: The URL of the server
    :param token: The token to authenticate the server
    :param db_uuid: The database UUID
    :param ore_code: The ore code
    """
    update_conf({
        'remote_server': url,
        'token': token,
...
@cache
def get_orj_server_url():
    """Get the URL of the linked Orj database.
    If the IoT Box is in access point mode, it will return ``None`` to avoid
    connecting to the server.

    :return: The URL of the linked Orj database.
    :rtype: str or None
    """
...
def get_commit_hash():
    return subprocess.run(
        ['git', '--work-tree=/home/pi/orj/', '--git-dir=/home/pi/orj/.git', 'rev-parse', '--short', 'HEAD'],
...
def get_version(detailed_version=False):
    if platform.system() == 'Linux':
        image_version = read_file_first_line('/var/orj/iotbox_version')
...
def load_certificate():
    """
    Send a request to Orj with customer db_uuid and ore_code to get a true certificate
    """
    db_uuid = get_conf('db_uuid')
    ore_code = get_conf('ore_code')
    if not (db_uuid and ore_code):
        return "ERR_IOT_HTTPS_LOAD_NO_CREDENTIAL"

    url = 'https://www.orj.net/orj-ore/iot/x509'
    data = {
        'params': {
            'db_uuid': db_uuid,
            'ore_code': ore_code
    ...
    try:
        ...
        _logger.exception("An error occurred while trying to reach orj.net servers.")
    ...
    if error:
        _logger.error("An error received from orj.net while trying to get the certificate: %s", error)
...
def delete_iot_handlers():
    ...
    try:
        ...
        filenames = [
            f"orj/addons/hw_drivers/iot_handlers/{file.relative_to(iot_handlers)}"
...
def download_iot_handlers(auto=True):
    """
    Get the drivers from the configured Orj server
    """
    server = get_orj_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                delete_iot_handlers()
                with writable():
                    path = path_file('orj', 'addons', 'hw_drivers', 'iot_handlers')
...
def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "orj.addons.hw_drivers.iot_handlers.{handler_kind}.{handler_name}".\
        format(handler_kind=handler_kind, handler_name=handler_file_name.removesuffix('.py'))

def load_iot_handlers():
    """
    This method loads local files: 'orj/addons/hw_drivers/iot_handlers/drivers' and
    'orj/addons/hw_drivers/iot_handlers/interfaces'
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows Orj
    server folder
    :return: The path to the file
    """
    platform_os = platform.system()
    if platform_os == 'Linux':
        return Path("~pi", *args).expanduser() # Path.home() returns orj user's home instead of pi's
    elif platform_os == 'Windows':
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Orj\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Orj\\downloaded_file.zip', 'C:\\Program Files\\Orj\\new_folder'))
...
def update_conf(values, section='iot.box'):
    """
    Update orj.conf with the given key and value.
    ...
    """
    _logger.debug("Updating orj.conf with values: %s", values)
    conf = get_conf()
    get_conf.cache_clear()  # Clear the cache to get the updated config

    if not conf.has_section(section):
        _logger.debug("Creating new section '%s' in orj.conf", section)
        conf.add_section(section)

    for key, value in values.items():
        conf.set(section, key, value) if value else conf.remove_option(section, key)

    write_file("orj.conf", conf)


@cache
def get_conf(key=None, section='iot.box'):
    """
    Get the value of the given key from orj.conf, or the full config if no key is provided.
...
def disconnect_from_server():
    ...
    get_orj_server_url.cache_clear()
    update_conf({
        'remote_server': '',
        'token': '',
        'db_uuid': '',
