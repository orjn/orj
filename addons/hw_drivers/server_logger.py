...
from orj.netsvc import DBFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in orj.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, orj_server_url, active):
        """
        :param orj_server_url: Orj Server URL
        """
        super().__init__()
        self._orj_server_url = orj_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._orj_server_url:
    ...
    def _periodic_flush(self):
        orj_session = requests.Session()
        while self._orj_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(orj_session)

    def _flush_logs(self, orj_session):
        ...
        if not self._orj_server_url or queue_size == 0:
            return
        try:
            orj_session.post(
                self._orj_server_url + '/iot/log',
                data=empty_queue(),
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_orj_server_url(), get_orj_config_log_to_server_option())
...
# Set it in the 'root' logger, on which every logger (including orj) is a child
