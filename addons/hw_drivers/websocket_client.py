...
from orj.addons.hw_drivers import main
...
def send_to_controller(device_type, params):
    """
    Confirm the operation's completion by sending a response back to the Orj server
    """
    routes = {
        "printer": "/iot/printer/status",
    }
    params['iot_mac'] = helpers.get_mac_address()
    server_url = helpers.get_orj_server_url() + routes[device_type]
    try:
...
class WebsocketClient(Thread):
    ...
    def run(self):
        self.ws = websocket.WebSocketApp(self.url,
            header={"User-Agent": "OrjIoTBox/1.0"},
