...
import { getRangePosition } from '@web_editor/js/editor/orj-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { closestScrollableY } from "@web/core/utils/scrolling";
import { useThrottleForAnimation } from "@web/core/utils/timing";
import { onWillUnmount, onWillStart, reactive, status, useSubEnv } from "@orj/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges(...args) {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.orjEditor.historyPauseSteps();
            ...
            this.wysiwyg.orjEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.orjEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.orjEditor.historyUnpauseSteps();
            this.wysiwyg.orjEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.orj-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.orjEditor.observerFlush();
        this.wysiwyg.orjEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Orj editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();
                // Update undo/redo buttons
                this.wysiwyg.orjEditor.dispatchEvent(new Event('historyStep'));

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.orjEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.orjEditor.editable.querySelector('p');
                if (p && selection) {
        ...
        if (editableAreaIsEmpty) {
            ...
            // We actually hide the OrjEditor toolbar by calling
        ...
        this.wysiwyg.orjEditor.activateContenteditable();
    }
    _getCodeViewEl() {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.orjEditor.resetContent($newLayout[0].outerHTML);
        ...
        this.onIframeUpdated();
        this.wysiwyg.orjEditor.historyStep(true);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await orj.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
