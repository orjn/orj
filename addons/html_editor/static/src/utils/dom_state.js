...
export function prepareUpdate(...args) {
    const closestRoot =
        args.length &&
        ancestors(args[0]).find((ancestor) => ancestor.classList.contains("orj-editor-editable"));
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, ".orj-editor-editable");
...
export function restoreState(prevStateData, debug = false) {
    ...
    if (debug) {
        const editable = closestElement(node, ".orj-editor-editable");
        console.log(
            "%c" +
