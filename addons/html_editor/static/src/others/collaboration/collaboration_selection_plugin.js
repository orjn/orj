...
export class CollaborationSelectionPlugin extends Plugin {
    ...
    static dependencies = ["history", "collaborationOrj", "position", "localOverlay"];
    resources = {
        /** Handlers */
    ...
    /**
     * @param {import("./collaboration_orj_plugin").CollaborationSelection} selection
     */
    ...
    drawPeerSelection({ selection, peerId }) {
        const peerMetadata = this.dependencies.collaborationOrj.getPeerMetadata(peerId);
        if (!peerMetadata) {
            return;
        }
