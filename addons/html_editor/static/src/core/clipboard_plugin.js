...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        const orjHtml = dataHtmlElement.innerHTML;
        const orjText = selection.textContent();
        ev.clipboardData.setData("text/plain", orjText);
        ev.clipboardData.setData("text/html", orjHtml);
        ev.clipboardData.setData("application/vnd.orj.orj-editor", orjHtml);
        if (commonAncestor && commonAncestor.nodeType === Node.ELEMENT_NODE) {
            this.dispatchTo("normalize_handlers", commonAncestor);
        }
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteOrjEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteOrjEditorHtml(clipboardData) {
        const orjEditorHtml = clipboardData.getData("application/vnd.orj.orj-editor");
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.orj.orj-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.orj.orj-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&
...
// @phoenix @todo: move to Orj plugin?
/**
