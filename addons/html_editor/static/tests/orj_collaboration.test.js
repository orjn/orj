...
import { advanceTime, animationFrame, tick, waitUntil } from "@orj/hoot-dom";
...
class PeerTest {
    ...
    async focus() {
        return this.plugins["collaborationOrj"].joinPeerToPeer();
    }
    ...
    async writeToServer() {
        ...
        const lastId = this.plugins.collaborationOrj.getLastHistoryStepId(
        ...
        for (const peer of Object.values(this.peers)) {
            ...
            peer.onlineMutex.exec(async () => {
                return peer.plugins.collaborationOrj.onServerLastIdUpdate(String(lastId));
            });
        }
    }
    async setOnline() {
...
class Wysiwygs extends Component {
    ...
    onLoad(peerId, editor) {
        ...
        editor.startPlugins = () => {
            ...
            patch(plugins["collaborationOrj"], {
                getMetadata() {
...
describe("Stale detection & recovery", () => {
    describe("detect stale while unfocused", () => {
        test("should do nothing until focus", async () => {
            ...
            expect(peers.p1.plugins.collaborationOrj.isDocumentStale).toBe(false, {
            ...
            expect(peers.p2.plugins.collaborationOrj.isDocumentStale).toBe(false, {
            ...
            expect(peers.p3.plugins.collaborationOrj.isDocumentStale).toBe(true, {
            ...
            expect(peers.p3.plugins.collaborationOrj.isDocumentStale).toBe(false, {
    ...
    describe("detect stale while focused", () => {
        describe("recover from missing steps", () => {
            test("should recover from missing steps", async () => {
                ...
                const p3Spies = makeSpies(peers.p3.plugins.collaborationOrj, [
                ...
                expect(peers.p1.plugins.collaborationOrj.historyShareId).toBe(
                    peers.p2.plugins.collaborationOrj.historyShareId,
                    {
                        message: "p1 and p2 should have the same historyShareId",
                    }
                );
                expect(peers.p1.plugins.collaborationOrj.historyShareId).toBe(
                    peers.p3.plugins.collaborationOrj.historyShareId,
                    {
        ...
        describe("recover from snapshot", () => {
            test("should wait for all peer to recover from snapshot", async () => {
                ...
                const p2Spies = makeSpies(peers.p2.plugins.collaborationOrj, [
                    "recoverFromStaleDocument",
                    "resetFromServerAndResyncWithPeers",
                    "processMissingSteps",
                    "applySnapshot",
...
describe("Disconnect & reconnect", () => {
    test("should sync history when disconnecting and reconnecting to internet", async () => {
        ...
        const p1PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p2.plugins.collaborationOrj, {
        ...
        const p2PromiseForMissingStep = new Promise((resolve) => {
            patch(peers.p1.plugins.collaborationOrj, {
                async processMissingSteps() {
                    super.processMissingSteps(...arguments);
                    resolve();
                },
...
describe("History steps Ids", () => {
    ...
    test("should clear history step ids when resetting from server", async () => {
        ...
        expect(peers.p2.plugins.collaborationOrj.isDocumentStale).toBe(true, {
