...
import orj.tests
from orj.tests.common import HttpCase, new_test_user
from orj.tools.json import scriptsafe as json_safe
...
class TestController(HttpCase):
    ...
    def test_05_internal_link_preview(self):
        ...
        response_without_preview_name = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": f"/orj/users/{self.portal_user.id}",
                }
            }),
        ...
        response_wrong_action = self.url_open(
            ...
            data=json_safe.dumps({
                "params": {
                    "preview_url": "/orj/actionInvalid/1",
                }
            }),
            headers=self.headers
        )
        ...
        # Attempt to retrieve metadata for path format `orj/<model>/<record_id>`
        response_model_record = self.url_open(
            '/html_editor/link_preview_internal',
            data=json_safe.dumps({
                "params": {
                    "preview_url": f"/orj/res.users/{self.portal_user.id}",
                }
            }),
            headers=self.headers
        )
