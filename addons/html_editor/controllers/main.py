...
from orj import _, http, tools, SUPERUSER_ID
from orj.addons.html_editor.tools import get_video_url_data
from orj.exceptions import UserError, MissingError, AccessError
...
DEFAULT_LIBRARY_ENDPOINT = 'https://media-api.orj.net'
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                # if path format is `orj/<model>/<record_id>` so we use `action_name` as model name
