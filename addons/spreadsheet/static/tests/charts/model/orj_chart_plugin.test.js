...
import { OrjBarChart } from "@spreadsheet/chart/orj_chart/orj_bar_chart";
import { OrjChart } from "@spreadsheet/chart/orj_chart/orj_chart";
import { OrjLineChart } from "@spreadsheet/chart/orj_chart/orj_line_chart";
...
test("Can add an Orj Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OrjBarChart).toBe(true);
...
test("Can add an Orj Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OrjLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Orj Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OrjChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Orj bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orj_bar",
...
test("Orj pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orj_pie",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Orj line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orj_line",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Can import/export an Orj chart", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "orj_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("orj_line");
...
test("Can undo/redo an Orj chart creation", async () => {
    const model = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "orj_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_pie" });
    insertChartInSpreadsheet(model, "orj_bar");
    insertChartInSpreadsheet(model, "orj_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Orj chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof OrjChart).toBe(true);
...
test("Can cut/paste Orj chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof OrjChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Orj chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load orj chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert orj chart from a different model", async () => {
    const model = await createModelWithDataSource();
...
test("Orj chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove orj chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getOrjChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getOrjChartIds().length).toBe(0);
});

test("Orj chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getOrjChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getOrjChartDisplayName(chartId)).toBe("(#1) Orj Line Chart");
});
...
test("Displays correct thousand separator for positive value in Orj Bar chart Y-axis", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const runtime = model.getters.getChartRuntime(chartId);
...
test("Thousand separator in Orj Bar chart Y-axis is locale-dependent", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orj_bar" });
