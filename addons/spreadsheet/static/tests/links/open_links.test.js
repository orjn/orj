...
test("click a web link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Orj](https://orj.net)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("https://orj.net");
    openLink(cell.link, env);
    expect.verifySteps(["https://orj.net"]);
});
...
test("click a menu link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](orj://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
test("click a menu link [2]", async () => {
    ...
    setCellContent(model, "A1", `[a view](orj://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("an orj view");
    openLink(cell.link, env);
...
test("Click a link containing an action xml id", async () => {
    ...
    const model = new Model({}, { custom: { env } });
    setCellContent(model, "A1", `[an action link](orj://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
