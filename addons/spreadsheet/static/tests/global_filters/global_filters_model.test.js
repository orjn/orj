...
import { animationFrame, mockDate, mockTimeZone } from "@orj/hoot-mock";

import { DispatchResult, Model, helpers, tokenize } from "@orj/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    const chartId = model.getters.getOrjChartIds()[0];
    fieldMatching = model.getters.getOrjChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("ORJ.FILTER.VALUE text filter", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A10", `=ORJ.FILTER.VALUE("Text Filter")`);
...
test("ORJ.FILTER.VALUE date filter", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A10", `=ORJ.FILTER.VALUE("Date Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
...
test("ORJ.FILTER.VALUE date from/to without values", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=ORJ.FILTER.VALUE("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe("");
});

test("ORJ.FILTER.VALUE date from/to with only from defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=ORJ.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("ORJ.FILTER.VALUE date from/to with only to defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=ORJ.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("ORJ.FILTER.VALUE date from/to with from and to defined", async function () {
    const model = await createModelWithDataSource();
    setCellContent(model, "A1", `=ORJ.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("ORJ.FILTER.VALUE relation filter", async function () {
    const model = await createModelWithDataSource({
    ...
    setCellContent(model, "A10", `=ORJ.FILTER.VALUE("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("ORJ.FILTER.VALUE with escaped quotes in the filter label", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=ORJ.FILTER.VALUE("my \\"special\\" filter")');
    expect(getCellValue(model, "A1")).toBe("Jean-Jacques");
});

test("ORJ.FILTER.VALUE formulas are updated when filter label is changed", async function () {
    const model = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(
        model,
        "A10",
        `=ORJ.FILTER.VALUE("Cuillère") & ORJ.FILTER.VALUE( "Cuillère" )`
    );
    const [filter] = model.getters.getGlobalFilters();
    const newFilter = {
        ...filter,
    ...
    expect(getCellFormula(model, "A10")).toBe(
        `=ORJ.FILTER.VALUE("Interprete") & ORJ.FILTER.VALUE("Interprete")`
    );
});
...
test("field matching is removed when list is deleted", async function () {
    ...
    model.dispatch("REMOVE_ORJ_LIST", { listId });
...
test("field matching is removed when an Orj chart is deleted", async function () {
    ...
    expect(model.getters.getOrjChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getOrjChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating the pivot domain should keep the global filter domain", async () => {
    ...
    model.dispatch("UPDATE_ORJ_PIVOT_DOMAIN", {
        pivotId,
        domain: [["foo", "in", [55]]],
    });
...
test("Updating a non-orj pivot should not crash on global filter", async () => {
    const grid = {
