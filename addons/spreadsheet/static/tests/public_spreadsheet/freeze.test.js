...
import { registries } from "@orj/o-spreadsheet";
...
import { freezeOrjData } from "@spreadsheet/helpers/model";
import { OrjPivot, OrjPivotRuntimeDefinition } from "@spreadsheet/pivot/orj_pivot";
...
test("orj pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeOrjData(model);
...
test("Pivot with a type different of ORJ is not converted", async function () {
    // Add a pivot with a type different of ORJ
...
test("orj pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeOrjData(model);
...
test("orj charts are replaced with an image", async function () {
...
test("orj links are replaced with their label", async function () {
    const view = {
        name: "an orj view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: { content: "[menu_xml](orj://ir_menu_xml_id/test_menu)" },
                    A2: { content: "[menu_id](orj://ir_menu_id/12)" },
                    A3: { content: `[orj_view](orj://view/${JSON.stringify(view)})` },
                    A4: { content: "[external_link](https://orj.net)" },
    ...
    const frozenData = await freezeOrjData(model);
    expect(frozenData.sheets[0].cells.A1.content).toBe("menu_xml");
    expect(frozenData.sheets[0].cells.A2.content).toBe("menu_id");
    expect(frozenData.sheets[0].cells.A3.content).toBe("orj_view");
    expect(frozenData.sheets[0].cells.A4.content).toBe("[external_link](https://orj.net)");
