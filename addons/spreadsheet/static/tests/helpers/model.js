 ...
 * @typedef {import("@spreadsheet/helpers/model").OrjSpreadsheetModel} OrjSpreadsheetModel
...
export function setupDataSourceEvaluation(model) {
    model.config.custom.orjDataProvider.addEventListener("data-source-updated", () => {
 ...
 * @returns {Promise<OrjSpreadsheetModel>}
 */
export async function createModelWithDataSource(params = {}) {
    ...
    const model = new Model(params.spreadsheetData, {
        ...
        custom: {
            env,
            orjDataProvider: new OrjDataProvider(env),
            ...config?.custom,
        },
    });
