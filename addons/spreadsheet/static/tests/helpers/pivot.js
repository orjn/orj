...
import { OrjPivot } from "@spreadsheet/pivot/orj_pivot";
...
export async function insertPivotInSpreadsheet(model, pivotId, params) {
    ...
    if (!(ds instanceof OrjPivot)) {
        throw new Error("The pivot data source is not an OrjPivot");
    }
    await ds.load();
 ...
 * @returns {Promise<{ model: OrjSpreadsheetModel, env: object, pivotId: string}>}
 */
