...
import { ORJ_VERSION } from "@spreadsheet/o_spreadsheet/migration";
import { Model, load } from "@orj/o-spreadsheet";
...
test("Orj formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
    ...
    expect(migratedData.sheets[0].cells.A3.content).toBe(`=ORJ.FILTER.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A4.content).toBe(`=ORJ.LIST("1")`);
    expect(migratedData.sheets[0].cells.A5.content).toBe(`=ORJ.LIST.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A6.content).toBe(`=ORJ.PIVOT.POSITION("1")`);
...
test("Pivot 'day' arguments are migrated", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=ORJ.PIVOT("1","21/07/2022")` },
                    A2: { content: `=ORJ.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=orj.pivot("1","21/07/2021")` },
                    A4: { content: `=ORJ.PIVOT("1","test")` },
                    A5: { content: `=orj.pivot("1","21/07/2021")+"21/07/2021"` },
...
test("Pivot formulas are migrated from 9 to 10", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=ORJ.PIVOT("1")` },
                    A2: { content: `=ORJ.PIVOT.HEADER("1")` },
                    A3: { content: `=ORJ.PIVOT.POSITION("1")` },
                    A4: { content: `=ORJ.PIVOT.TABLE("1")` },
                    A5: { content: `=orj.pivot("1")` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1.content).toBe(`=PIVOT.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A2.content).toBe(`=PIVOT.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A3.content).toBe(`=ORJ.PIVOT.POSITION("1")`);
...
test("Pivot formulas using pivot positions are migrated (11 to 12)", () => {
        ...
        sheets: [
            {
                cells: {
                    A1: {
                        content: `=-PIVOT.VALUE("1","balance","account_id",ORJ.PIVOT.POSITION("1","account_id",12),"date:quarter","4/"&ORJ.FILTER.VALUE("Year"))`,
                    },
                    A2: {
                        content: `=PIVOT.HEADER("1","account_id",ORJ.PIVOT.POSITION("1","account_id",14))`,
                    },
                    A3: { content: `=ORJ.PIVOT.POSITION("1","account_id",14)` },
                    A4: { content: `=ORJ.PIVOT.POSITION("1",14)` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1.content).toBe(
        `=-PIVOT.VALUE("1","balance","#account_id",12,"date:quarter","4/"&ORJ.FILTER.VALUE("Year"))`
    );
    ...
    expect(migratedData.sheets[0].cells.A3.content).toBe(
        `=ORJ.PIVOT.POSITION("1","account_id",14)`
    );
    expect(migratedData.sheets[0].cells.A4.content).toBe(`=ORJ.PIVOT.POSITION("1",14)`);
});

test("Orj version is exported", () => {
    const model = new Model();
    expect(model.exportData().orjVersion).toBe(ORJ_VERSION);
});
