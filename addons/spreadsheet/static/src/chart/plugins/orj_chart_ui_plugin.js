...
export class OrjChartUIPlugin extends OrjUIPlugin {
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getOrjChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getOrjChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getOrjChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getOrjChartDataSourceId(chartId);
        return this.charts[dataSourceId];
    }

    /**
     * Get the callback used for onClick and onHover in an Orj Chart
     */
     ...
     * @return {Promise[]}
     */
    _getOrjChartsWaitForReady() {
        return this.getters
            .getOrjChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getOrjChartDataSourceId(chartId) {
