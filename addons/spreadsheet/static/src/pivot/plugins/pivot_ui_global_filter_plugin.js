...
export class PivotUIGlobalFilterPlugin extends OrjUIPlugin {
     ...
     */
    _addDomain(pivotId) {
        if (this.getters.getPivotCoreDefinition(pivotId).type !== "ORJ") {
            return;
        }
        const domainList = [];
    ...
    _addDomains() {
        for (const pivotId of this.getters
            .getPivotIds()
            .filter((pivotId) => this.getters.getPivot(pivotId).type === "ORJ")) {
