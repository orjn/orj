...
export class ListUIPlugin extends OrjUIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "INSERT_ORJ_LIST": {
                const { id, linesNumber } = cmd;
                this._setupListDataSource(id, linesNumber);
                break;
            }
            case "INSERT_ORJ_LIST_WITH_TABLE": {
                this.dispatch("INSERT_ORJ_LIST", cmd);
                this._addTable(cmd);
                break;
            }
            case "RE_INSERT_ORJ_LIST_WITH_TABLE": {
                this.dispatch("RE_INSERT_ORJ_LIST", cmd);
                this._addTable(cmd);
                break;
            }
            case "DUPLICATE_ORJ_LIST": {
                this._setupListDataSource(cmd.newListId, 0);
                break;
            }
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshOrjLists();
                break;
            ...
            case "CLEAR_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_ORJ_LIST":
            case "UPDATE_ORJ_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const updateCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_ORJ_LIST_DOMAIN" ||
                        cmd.type === "UPDATE_ORJ_LIST" ||
                        cmd.type === "INSERT_ORJ_LIST"
                );
