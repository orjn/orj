...
import { EvaluationError, helpers, registries } from "@orj/o-spreadsheet";
...
export class CurrencyPlugin extends OrjUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.orjDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'OrjDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("orjCurrency", CurrencyPlugin);
