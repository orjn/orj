...
export default class DevicesSynchronisation {
    ...
    async collect(data) {
        ...
        if (orj.debug === "assets") {
            ...
            console.info("Login number", orj.login_number, login_number);
            console.info("Session Ids", orj.pos_session_id, session_id);
        }

        if (login_number == orj.login_number || session_id != orj.pos_session_id) {
            return;
        }
    ...
    constructOrdersDomain(serverOpenOrders) {
        ...
        domain = Domain.and([
            ...
            new Domain([
                "|",
                ["config_id", "=", orj.pos_config_id],
                [
                    "config_id",
                    "in",
                    this.models["pos.config"].get(orj.pos_config_id).raw.trusted_config_ids,
                ],
            ]),
        ]);
