...
function connect () {
	...
	current_iotbox_version=$(cat "/var/orj/iotbox_version")
	...
	if [ ${TIMEOUT_RETURN} -eq 124 ] && [ -z "${NO_AP}" ] ; then
		...
		sudo /home/pi/orj/addons/point_of_sale/tools/posbox/configuration/wireless_ap.sh "force" &
	else
		...
		if [ ! -f "${LOST_WIFI_FILE}" ] ; then
			logger -t posbox_connect_to_wifi "Restarting orj"
			sudo service orj restart
		fi
		...
		/home/pi/orj/addons/point_of_sale/tools/posbox/configuration/keep_wifi_alive.sh &
	fi
}
