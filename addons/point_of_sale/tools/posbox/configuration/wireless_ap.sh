...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ "${ESSID}" ] && [ "${PASSWORD}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/orj/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		service orj restart # As this file is executed on boot, this line is responsible for restarting orj service on reboot
	fi
# wired
else
	killall nginx
