...
from orj.addons.payment.controllers.portal import PaymentPortal
...
class TestFlows(PaymentHttpCommon):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger("orj.http"), self.assertRaises(
            JsonRpcException,
            msg='orj.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.',
        ):
    ...
    def test_transaction_wrong_token(self):
        ...
        with mute_logger('orj.http'), self.assertRaises(JsonRpcException, msg='orj.exceptions.ValidationError: The access token is invalid.'):
