...
function OrjEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
OrjEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
OrjEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Orj where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new OrjEvent(this, name, info);
