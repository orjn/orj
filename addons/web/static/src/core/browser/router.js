...
export function stateToUrl(state) {
    ...
    const start_url = isScopedApp() ? "scoped_app" : "orj";
 ...
 * external website). Unfortunately, Orj wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/orj") &&
            (["/web", "/orj"].includes(url.pathname) || url.pathname.startsWith("/orj/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/orj") && url.hash) {
