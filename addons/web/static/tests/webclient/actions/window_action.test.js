...
import { click, queryAllTexts, waitFor } from "@orj/hoot-dom";
import { Deferred, animationFrame, runAllTimers } from "@orj/hoot-mock";
import {
    MockServer,
    contains,
    createKanbanRecord,
...
test("A new form view can be reloaded after a failed one", async () => {
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/orj/action-3/1");
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/orj/action-3/2");
    ...
    defineActions([
        {
            ...
            views: [
                [false, "list"],
                [false, "unknown"], // typically, an ore-only view on a community db
                [false, "kanban"],
                [false, "form"],
            ],
        },
...
test("flags field of ir.actions.act_window is used", async () => {
    // more info about flags field : https://github.com/orj/orj.netmit/c9b133813b250e89f1f61816b0eabfb9bee2009d
    defineActions([
        {
            id: 43,
            name: "Partners",
