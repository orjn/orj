...
import { animationFrame, runAllTimers } from "@orj/hoot-mock";
...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /orj?test=42",
        "window_reload",
        "pushState /orj/action-2",
        "window_reload",
        "pushState /orj?menu_id=1",
        "window_reload",
        "pushState /orj/action-1?menu_id=2",
        "window_reload",
    ]);
});
