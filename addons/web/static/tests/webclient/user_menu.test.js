...
import { click, queryAllAttributes, queryAllProperties, queryAllTexts } from "@orj/hoot-dom";
...
import { orjAccountItem, preferencesItem } from "@web/webclient/user_menu/user_menu_items";
...
test("click on orj account item", async () => {
    patchWithCleanup(browser, {
        open: (url) => expect.step(`open ${url}`),
    });
    userMenuRegistry.add("orj_account", orjAccountItem);
    await mountWithCleanup(UserMenu);
    onRpc("/web/session/account", () => "https://account-url.com");
    stepAllNetworkCalls();
    await contains("button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveCount(1);
    expect(".o-dropdown--menu .dropdown-item").toHaveText("My Orj.net account");
