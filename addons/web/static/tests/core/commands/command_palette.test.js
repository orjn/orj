...
import { press, queryAll, queryAllTexts, queryOne } from "@orj/hoot-dom";
import { Deferred, advanceTime, animationFrame, runAllTimers } from "@orj/hoot-mock";
import {
    contains,
    getService,
    mountWithCleanup,
...
test("checks that href is correctly used", async () => {
    ...
    expect(".o_command_palette .o_command:eq(0) a").toHaveAttribute("href", "https://www.orj.net");
    // Check that we get url when doing ctrl+enter on a command having a link inside it
    await press("control+enter");
    await animationFrame();
    expect.verifySteps(["https://www.orj.net"]);
