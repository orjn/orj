...
import { describe, test, expect } from "@orj/hoot";
...
import { click, freezeTime, queryAllTexts } from "@orj/hoot-dom";
import {
    ClientErrorDialog,
    Error504Dialog,
    ErrorDialog,
...
test("WarningDialog", async () => {
    ...
    await mountWithCleanup(WarningDialog, {
        env,
        props: {
            exceptionName: "orj.exceptions.UserError",
            message: "...",
...
test("RedirectWarningDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Orj Warning");
    expect("main").toHaveText("Some strange unreadable message");
...
test("SessionExpiredDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Orj Session Expired");
    expect("main p").toHaveText(
        "Your Orj session expired. The current page is about to be refreshed."
    );
    expect(".o_dialog footer button").toHaveText("Close");
