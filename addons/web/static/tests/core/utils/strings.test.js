...
test("escape", () => {
    ...
    expect(escape(`<a href="https://www.orj.net">orj<a>`)).toBe(
        `&lt;a href=&quot;https://www.orj.net&quot;&gt;orj&lt;a&gt;`
    );
    expect(escape(`<a href='https://www.orj.net'>orj<a>`)).toBe(
        `&lt;a href=&#x27;https://www.orj.net&#x27;&gt;orj&lt;a&gt;`
    );
    expect(escape("<a href='https://www.orj.net'>Orj`s website<a>")).toBe(
        `&lt;a href=&#x27;https://www.orj.net&#x27;&gt;Orj&#x60;s website&lt;a&gt;`
    );
});
...
test("escapeRegExp", () => {
    ...
    expect(escapeRegExp("[wowl]")).toBe("\\[wowl\\]");
    expect(escapeRegExp("[wowl.orj]")).toBe("\\[wowl\\.orj\\]");
    expect(escapeRegExp("^orj.define([.]*)$")).toBe("\\^orj\\.define\\(\\[\\.\\]\\*\\)\\$");
    expect(escapeRegExp("[.*+?^${}()|[]\\")).toBe("\\[\\.\\*\\+\\?\\^\\$\\{\\}\\(\\)\\|\\[\\]\\\\");
});
...
test("isEmail", () => {
    ...
    expect(isEmail("test@orj")).toBe(false);
    expect(isEmail("test@orj@orj.net")).toBe(false);
    expect(isEmail("te st@orj.net")).toBe(false);

    expect(isEmail("test@orj.net")).toBe(true);
});
