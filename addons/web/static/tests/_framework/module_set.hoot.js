...
import { describe, dryRun, globals, start, stop } from "@orj/hoot";
import { Deferred } from "@orj/hoot-dom";
import { watchKeys, watchListeners } from "@orj/hoot-mock";
import { whenReady } from "@orj/owl";
...
class ModuleSetLoader extends loader.constructor {
    ...
    constructor(moduleSet) {
        ...
        orj.define = this.define.bind(this);
        orj.loader = this;
    }
...
const AUTO_INCLUDED_ADDONS = {
    /**
     * spreadsheet addons defines a module that does not starts with `@spreadsheet` but `@orj` (`@orj/o-spreadsheet)
     * To ensure that this module is loaded, we have to include `orj` in the dependencies
     */
    spreadsheet: ["orj"],
    /**
     * Add all view types by default
     */
    web_ore: ["web_gantt", "web_grid", "web_map"],
};
const CSRF_TOKEN = orj.csrf_token;
...
const R_DEFAULT_MODULE = /^@orj\/(owl|hoot)/;
