...
function invalidImportsFrom(folder, allowedFolders) {
    ...
    const modulesToCheck = Array.from(orj.loader.modules.keys()).filter((module) =>
        module.startsWith(`@web/${folder}/`)
    );
    const invalidDeps = {};
    for (const module of modulesToCheck) {
        const invalid = orj.loader.factories.get(module).deps.filter((dep) => {
            // owl and @web/session are allowed everywhere
            if (dep === "@orj/owl" || dep === "@web/session") {
                return false;
            }
