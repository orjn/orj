...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "Orj")
        self.assertEqual(data["scope"], "/orj")
        self.assertEqual(data["start_url"], "/orj")
        self.assertEqual(data["display"], "standalone")
        self.assertEqual(data["background_color"], "#1c64be")
        self.assertEqual(data["theme_color"], "#1c64be")
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/orj-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/orj-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
        ...
        for shortcut in data["shortcuts"]:
            ...
            self.assertTrue(shortcut["url"].startswith("/orj?menu_id="))
    ...
    def test_webmanifest_scoped(self):
        ...
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': "/web/static/img/orj-icon-192x192.png", 'sizes': 'any', 'type': 'image/png'}
        ])
        self.assertEqual(len(data["shortcuts"]), 0)
    ...
    def test_serviceworker(self):
        ...
        self.assertEqual(response.headers["Service-Worker-Allowed"], "/orj")

    def test_offline_url(self):
        """
        This route returns the offline page
        """
        response = self.url_open("/orj/offline")
        self.assertEqual(response.status_code, 200)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/orj-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/orj")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/orj-icon-ios.png"/>', document.text,
