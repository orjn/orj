...
class TransifexTranslation(models.AbstractModel):
    ...
    def _get_transifex_projects(self):
        ...
        first section is [main], after '[orj-16.sale]'
        ...
        projects = {}
        for addon_path in orj.addons.__path__:
            for tx_path in (
                ...
                if isfile(tx_path):
                    ...
                    for sec in tx_config_file.sections()[1:]:
                        if len(sec.split(":")) != 6:
                            # old format ['main', 'orj-16.base', ...]
                            tx_project, tx_mod = sec.split(".")
                        else:
                            # tx_config_file.sections(): ['main', 'o:orj:p:orj-16:r:base', ...]
    ...
    def _update_transifex_url(self, translations):
        ...
        # e.g. 'https://www.transifex.com/orj/'
        ...
        for translation in translations:
            ...
            # e.g. https://www.transifex.com/orj/orj-16/translate/#fr_FR/sale/42?q=text:'Sale+Order'
