...
import logging
import requests
from orj.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
from datetime import timedelta

from orj import api, fields, models, _, Command
from orj.exceptions import UserError
from orj.loglevels import exception_to_unicode
from orj.addons.microsoft_account.models import microsoft_service
from orj.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Orj
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2orj(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Orj -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_orj2microsoft()
        ...
        (events - synced_events)._sync_orj2microsoft()
        self.sudo().microsoft_last_sync_date = fields.datetime.now()
    ...
    def _set_ICP_first_synchronization_date(self, now):
        ...
        created Orj events and thus avoid spamming invitations for those events.
        """
