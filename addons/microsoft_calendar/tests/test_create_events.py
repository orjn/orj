...
from orj.addons.mail.tests.common import MailCommon, mail_new_test_user
from orj.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from orj.addons.microsoft_calendar.utils.microsoft_event import MicrosoftEvent
from orj.addons.microsoft_calendar.models.res_users import User
from orj.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Orj event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_without_email(self):
        ...
        with self.assertRaises(ValidationError):
            record._sync_orj2microsoft()

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Orj organizer calendar.
        """
        ...
        self.assert_orj_event(new_records, self.expected_orj_event_from_outlook)
        self.assertEqual(new_records.user_id, self.organizer_user)
        self.assertEqual(new_records.need_sync_m, False)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_orj(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Orj attendee calendar.
        There is a Orj user that matches with the organizer email address.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_orj(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Orj attendee calendar.
        no Orj user that matches with the organizer email address.
        """
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@orj.net", 'name': "John Doe"},
        })
        expected_event = dict(self.expected_orj_event_from_outlook, user_id=False)
        ...
        new_records = (records - existing_records)
        self.assertEqual(len(new_records), 1)
        self.assert_orj_event(new_records, expected_event)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Orj attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Orj.
        """
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Orj recurrent event is created when Outlook sync is not enabled.
        """
        if not self.sync_orj_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync(self, mock_insert, mock_get_events):
        """
        A Orj recurrent event is created when Outlook sync is enabled.
        """
        if not self.sync_orj_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A Orj recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Orj organizer calendar.
        """
        ...
        self.assert_orj_recurrence(new_recurrences, self.expected_orj_recurrency_from_outlook)
        for i, e in enumerate(sorted(new_events, key=lambda e: e.id)):
            self.assert_orj_event(e, self.expected_orj_recurrency_events_from_outlook[i])

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Orj attendee calendar.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Orj due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Orj Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Orj Calendar synced.
        ...
        # In Orj, this event will be created (behind the screens) by a synced Orj user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Orj through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Orj (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Orj.
        """
    ...
    def test_create_event_with_default_and_undefined_sensitivity(self):
        """ Check if microsoft events are created in Orj when 'None' sensitivity setting is defined and also when it is not. """
        # Sync events from Microsoft to Orj after adding the sensitivity (privacy) property.
        ...
        self.env['calendar.event']._sync_microsoft2orj(MicrosoftEvent([undefined_privacy_event, default_privacy_event]))

        # Ensure that synced events have the correct privacy field in Orj.
        undefined_privacy_orj_event = self.env['calendar.event'].search([('microsoft_id', '=', 100)])
        default_privacy_orj_event = self.env['calendar.event'].search([('microsoft_id', '=', 200)])
        self.assertFalse(undefined_privacy_orj_event.privacy, "Event with undefined privacy must have False value in privacy field.")
        self.assertFalse(default_privacy_orj_event.privacy, "Event with custom privacy must have False value in privacy field.")

    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_create_videocall_sync_microsoft_calendar(self, mock_insert, mock_get_events):
        """
        Test syncing an event from Orj to Microsoft Calendar.
        ...
        # Verify the event's videocall_location is updated in Orj
    ...
    def test_skip_sync_for_non_synchronized_users_new_events(self, mock_insert):
        ...
        Orj, as well cause problems in the future the synchronization of that event for the original owner.
        """
        ...
        self.assertTrue(event, "The event for the not synchronized owner must be created in Orj.")

        # Synchronize the calendar of user A, then make sure insert was not called.
        event.with_user(self.organizer_user).sudo()._sync_orj2microsoft()
        mock_insert.assert_not_called()
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_new_db_skip_orj2microsoft_sync_previously_created_events(self, mock_insert, mock_get_events):
        """
        ...
        # Ten seconds later the ICP parameter saving, make the synchronization between Orj
        ...
        with self.mock_datetime_and_now('2024-01-02 10:00:10'):
            # Mock the return of 0 events from Outlook to Orj, then activate the user's sync.
            mock_get_events.return_value = ([], None)
            self.organizer_user.microsoft_synchronization_stopped = False
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_old_db_orj2microsoft_sync_previously_created_events(self, mock_insert, mock_get_events):
        """
...
class TestSyncOrj2MicrosoftMail(TestCommon, MailCommon):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
