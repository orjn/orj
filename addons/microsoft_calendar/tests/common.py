...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Orj event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Orj events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Orj events
        # -----------------------------------------------------------------------------------------

        self.expected_orj_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_orj_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_orj_recurrency_events_from_outlook = [
            {
    ...
    def sync_orj_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Orj. A trick for deactivating the checking
    ...
    def assert_orj_event(self, orj_event, expected_values):
        """
        Assert that an Orj event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        orj_event_values = orj_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(orj_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, orj_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_orj_recurrence(self, orj_recurrence, expected_values):
        """
        Assert that an Orj recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        orj_recurrence_values = orj_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, orj_recurrence_values.get(k), msg=f"'{k}' mismatch")
