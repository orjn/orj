...
class AccountMove(models.Model):
    ...
    def action_l10n_vn_edi_update_payment_status(self):
        ...
        for invoice in invoices:
            ...
            # SInvoice will return a NOT_FOUND_DATA error if the status in Orj matches the one on their side.
        ...
        try:
            ...
            self.with_context(no_new_invoice=True).message_post(
                body=_('The invoice has been canceled on sinvoice for reason: %(reason)s'
                       'But the cancellation in Orj failed with error: %(error)s', reason=reason, error=e),
            )
