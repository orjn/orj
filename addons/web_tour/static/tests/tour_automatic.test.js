...
import { afterEach, beforeEach, describe, expect, test } from "@orj/hoot";
import { advanceTime, animationFrame, queryFirst } from "@orj/hoot-dom";
...
test("a tour with invalid step trigger", async () => {
    ...
    await orj.startTour("tour_invalid_trigger", { mode: "auto" }); // Use orj to run tour from registry because this is a test tour
    await waitForMacro();
    const expectedSteps = [
...
test("a failing tour logs the step that failed in run", async () => {
    ...
    await orj.startTour("tour2", { mode: "auto" }); // Use orj to run tour from registry because this is a test tour
    await waitForMacro();
    const expectedError = [
...
test("a failing tour with disabled element", async () => {
    ...
    await orj.startTour("tour3", { mode: "auto" });
    await waitForMacro();
    const expectedError = [
...
test("a failing tour logs the step that failed", async () => {
    ...
    await orj.startTour("tour1", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("check tour with inactive steps", async () => {
    ...
    await orj.startTour("pipu_tour", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("automatic tour with invisible element", async () => {
    ...
    await orj.startTour("tour_de_wallonie", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("automatic tour with alternative trigger", async () => {
    ...
    await mountWithCleanup(Root);
    await orj.startTour("tour_des_flandres", { mode: "auto" });
    await waitForMacro();
...
test("check not possible to click below modal", async () => {
    ...
    await orj.startTour("tour_check_modal", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("a tour where hoot trigger failed", async () => {
    ...
    await orj.startTour("tour_hoot_failed", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
