...
import { queryAll, queryFirst, queryOne } from "@orj/hoot-dom";
import { Component, useState, useExternalListener } from "@orj/owl";
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasOrjClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasOrjClass;
        ...
        const orjClass = [...currentElem.classList].find((c) => c.match(ORJ_CLASS_REGEX));
        if (orjClass) {
            currentPredicate = `.${orjClass}`;
            hasOrjClass = true;
        }
