...
from orj.exceptions import AccessDenied, AccessError, UserError
from orj.http import request
from orj.modules.module import adapt_version, MANIFEST_NAMES
from orj.osv.expression import is_leaf
from orj.release import major_version
from orj.tools import convert_csv_import, convert_sql_import, convert_xml_import, exception_to_unicode
from orj.tools import file_open, file_open_temporary_directory, ormcache

_logger = logging.getLogger(__name__)

APPS_URL = "https://apps.orj.net"
...
class IrModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            ...
            raise UserError(_("Studio customizations require the Orj Studio app."))
    ...
    def _get_missing_dependencies(self, zip_data):
        ...
        if unavailable_modules:
            ...
            for module in unavailable_modules:
                description += "- " + module + "\n"
            description += _(
                "\nYou may need the Ore version to install the data module. Please visit "
                "https://www.orj.net/pricing-plan for more information.\n"
                "If you need Website themes, it can be downloaded from https://github.com/orj/design-themes.\n"
            )
        else:
            description = _(
