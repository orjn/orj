...
from orj.service.server import CommonServer
from orj.tools import json_default, SQL
...
def get_notify_payload_max_length(default=8000):
    try:
        length = int(os.environ.get('ORJ_NOTIFY_PAYLOAD_MAX_LENGTH', default))
    except ValueError:
        _logger.warning("ORJ_NOTIFY_PAYLOAD_MAX_LENGTH has to be an integer, "
                ...
                with orj.sql_db.db_connect("postgres").cursor() as cr:
                    for payload in payloads:
                        cr.execute(
                            SQL(
                                "SELECT %s('imbus', %s)",
                                SQL.identifier(ORJ_NOTIFY_FUNCTION),
                                payload,
                            )
                        )
