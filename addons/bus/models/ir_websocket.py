...
class IrWebsocket(models.AbstractModel):
    ...
    def _prepare_subscribe_data(self, channels, last):
        ...
        :return:
            ...
            - missed_presences (orj.models.Recordset): The missed presences.
        ...
        str_presence_channels = {
            c for c in channels if isinstance(c, str) and c.startswith("orj-presence-")
        }
        presence_channels = self._build_presence_channel_list(
            [tuple(c.replace("orj-presence-", "").split("_")) for c in str_presence_channels]
        )
