...
from orj import SUPERUSER_ID, Command
from orj.fields import Date
from orj.tests import Form, tagged, freeze_time
from orj.tests.common import TransactionCase
from orj.tools import format_date
from orj.tools.date_utils import add
...
class TestReorderingRule(TransactionCase):
    ...
    def test_multi_lingual_orderpoints(self):
        ...
        self.assertEqual(len(po_line.order_id.order_line), 1)
        # the moves_dest_ids are not expected to be merged since the scheduler is excuted by roborj in en_US rather fr_FR
