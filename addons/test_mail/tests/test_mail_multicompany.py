...
from orj.addons.test_mail.models.test_mail_corner_case_models import MailTestMultiCompanyWithActivity
from orj.addons.test_mail.tests.common import TestRecipients
from orj.exceptions import AccessError
from orj.tests import tagged, users, HttpCase
...
class TestMailMCCommon(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.fake_email = cls.env['mail.message'].create({
            ...
            'message_id': '<123456-orj-%s-mail.test.gateway@%s>' % (cls.test_record.id, socket.gethostname()),
        })
...
class TestMultiCompanySetup(TestMailMCCommon, HttpCase):
    ...
    @users('employee_c2')
    @mute_logger('orj.addons.base.models.ir_rule')
            ...
            with self.subTest(login=login, mc_record=mc_record):
                ...
                if not login:
                    ...
                    if mc_error:
                        ...
                        self.assertEqual(parsed_url.path, '/orj/action-mail.action_discuss')
                    else:
