...
from orj.tests.common import tagged, users, HttpCase
from orj.tools import is_html_empty, mute_logger, formataddr
...
class TestMessageValues(MailCommon):
    ...
    @mute_logger('orj.models.unlink', 'orj.addons.mail.models.models')
    @users('employee')
    ...
    def test_mail_message_values_fromto_document_alias(self):
        ...
        self.assertIn('-orj-%d-mail.test' % self.alias_record.id, msg.message_id.split('@')[0])
    ...
    def test_mail_message_values_fromto_document_no_alias(self):
        ...
        self.assertIn('-orj-%d-mail.test.simple' % test_record.id, msg.message_id.split('@')[0])
...
class TestMessageLinks(MailCommon, HttpCase):
    ...
    def test_message_link_by_employee(self):
        ...
        with self.subTest(thread_message=thread_message):
            expected_url = self.base_url() + f'/orj/{thread_message.model}/{thread_message.res_id}?highlight_message_id={thread_message.id}'
