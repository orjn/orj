...
from orj import exceptions
from orj.addons.mail.models.mail_message import Message
from orj.addons.mail.models.mail_thread import MailThread
from orj.addons.mail.tests.common import mail_new_test_user, MailCommon
from orj.addons.test_mail.data import test_mail_data
from orj.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE, THAI_EMAIL_WINDOWS_874
from orj.addons.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups, MailTestTicket
from orj.sql_db import Cursor
from orj.tests import Form, tagged, RecordCapturer
from orj.tools import mute_logger
from orj.tools.mail import email_split_and_format, formataddr
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_binary_octetstream(self):
        ...
        with self.assertLogs('orj.addons.mail.models.mail_thread', level="WARNING") as capture:
        ...
        self.assertEqual(capture.output, [
            ("WARNING:orj.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_eml(self):
        ...
        mail = self.format(test_mail_data.MAIL_EML_ATTACHMENT, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>', to=f'generic@{self.alias_domain}',
                           msg_id='<cb7eaf62-58dc-2017-148c-305d0c78892f@orj.net>',
                           references='<f3b9f8f8-28fa-2543-cab2-7aa68f679ebb@orj.net>',
    ...
    def test_message_parse_eml_bounce_headers(self):
        ...
        msg_id = '<861878175823148.1577183525.736005783081055-orj-19177-account.invoice@mycompany.example.com>'
        mail = self.format(
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        message = self.env['mail.message'].create({
            'message_id' : '<368396033905967.1673346177.695352554321289-orj-11-sale.order@eupp00>'
        })
        incoming_bounce = self.format(
            test_mail_data.MAIL_BOUNCE_QP_RFC822_HEADERS,
            email_from='MAILER-DAEMON@mailserver.orj.net (Mail Delivery System)',
            email_to='bounce@xxx.orj.net',
            delivered_to='bounce@xxx.orj.net'
        )
...
class MailGatewayCommon(MailCommon):
    ...
    def _create_gateway_message(cls, record, msg_id_prefix, **values):
        msg_values = {
            ...
            'message_id': f'<{msg_id_prefix}-orj-{record.id}-{record._name}@{socket.gethostname()}>',
            'message_type': 'email',
            'model': record._name,
            'res_id': record.id,
...
class TestMailgateway(MailGatewayCommon):
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.models')
    ...
    @mute_logger('orj.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not orjbot:
        ...
        orjbot.active = True
        orjbot.email = 'orjbot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, orjbot.email_formatted, f'groups@{self.alias_domain}',
                subject='Orjbot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, orjbot)
        self.assertEqual(record4.message_ids[0].email_from, orjbot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: orjbot -> no follower')
    ...
    @mute_logger('orj.addons.mail.models.mail_mail', 'orj.addons.mail.models.mail_thread', 'orj.models.unlink', 'orj.tests')
    ...
    @mute_logger('orj.addons.mail.models.mail_mail', 'orj.addons.mail.models.mail_thread', 'orj.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on orjbot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Orjbot should be creating the message')
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.models.unlink', 'orj.addons.mail.models.mail_mail')
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.addons.mail.models.mail_mail', 'orj.models.unlink')
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.addons.mail.models.mail_mail', 'orj.models', 'orj.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('orj.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('orj.addons.mail.models.mail_alias.Alias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.models', 'orj.tests')
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.models.unlink')
    ...
    @mute_logger('orj.addons.mail.models.mail_thread', 'orj.models.unlink', 'orj.addons.mail.models.mail_mail', 'orj.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by orjbot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but orjbot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by orj should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        orj should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since Orj saas-3 """
        self.assertRaises(ValueError,
                          self.format_and_process, MAIL_TEMPLATE,
                          self.partner_1.email_formatted, f'noone@{self.alias_domain}', subject='spam',
                          extra=f'In-Reply-To: <12321321-orj-{self.test_record.id}-{self.test_record._name}@{socket.gethostname()}>')

        # when 6.1 messages are present, compat mode is available
        # Orj 10 update: compat mode has been removed and should not work anymore
        ...
        self.assertRaises(
            ...
            self.format_and_process, MAIL_TEMPLATE,
            self.partner_1.email_formatted, f'noone@{self.alias_domain}>', subject='spam',
            extra=f'In-Reply-To: <12321321-orj-{self.test_record.id}-mail.test.gateway@{socket.gethostname()}>')
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Orj is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    @mute_logger('orj.addons.mail.models.mail_mail', 'orj.addons.mail.models.mail_thread')
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Orj servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Orj server -> msg_id is
...
class TestMailGatewayReplies(MailGatewayCommon):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{email.message_id} {log.message_id} {reply.message_id}',  # should contain reference to OrjExternal message, logs to fill up history
                },
                'mail_mail_values': {
    ...
    def test_routing_reply_internal_messages(self):
        """ Test routing notably between two Orjs when internal messages
        ...
        RFQ-like                                        creation log
                                                        initial_msg
        Orj2 replies           creation log
                                reply                   reply
        -some internal work-                            user_notification
        Orj1 replies           reply_2                 reply_2 (incoming email)
        -some internal work-                            log
        Orj2 replies           reply_3                 reply_3 (outgoing email)

        Purpose: have references from Orj2 containing message IDs to try to
        ...
        gateway_record.message_post(
            author_id=self.env.ref('base.partner_root').id,
            body='OrjExternal Inquiry',
            email_from=self.partner_1.email_normalized,
            message_type='comment',
            subtype_id=self.env.ref('mail.mt_comment').id,
        )
        self.assertEqual(gateway_record.message_partner_ids, self.partner_admin)
        log, orjext_msg = gateway_record.message_ids[1], gateway_record.message_ids[0]
        self.assertEqual(orjext_msg.parent_id, log, 'Log serves as thread ancestor')
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{log.message_id} {orjext_msg.message_id} {reply.message_id}',  # should contain reference to OrjExternal message
                },
                'mail_mail_values': {
        ...
        # coming from Orj1: their reply as an incoming email
        with self.mock_mail_gateway():
            self.format_and_process(
                MAIL_TEMPLATE, self.email_from, reply.reply_to,
                subject='Gateway Creation',
                extra=f'References: {reply.message_id} <msg1@orj1>',
                debug_log=True,
            )
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{log.message_id} {orjext_msg.message_id} {reply.message_id} {reply_2.message_id}',  # should contain reference to OrjExternal message
                },
                'mail_mail_values': {
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{orjext_msg.message_id} {reply.message_id} {reply_2.message_id} {reply_3.message_id}',  # should contain reference to OrjExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
                    'parent_id': log,  # log serves as thread ancestor
