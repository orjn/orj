...
from orj.tools import SQL, mute_logger
...
class PasskeyTest(HttpCaseWithUserDemo):
    ...
    def test_check_identity(self):
        for key in ['test-yubikey', 'test-yubikey-nano', 'test-keepassxc']:
            ...
            with self.patch_start_auth(webauthn_challenge):
                ...
                self.assertEqual(response['error']['data']['name'], 'orj.exceptions.UserError')
                self.assertEqual(
    ...
    def test_check_user_verification(self):
        ...
        with self.patch_start_auth(webauthn_challenge):
            ...
            # Login successful, redirected to /orj
            self.assertTrue(response.url.endswith('/orj'))
